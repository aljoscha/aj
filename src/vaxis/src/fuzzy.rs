...
mod tests {
    ...
    #[test]
    fn exact_match_outranks_longer_partial_match() {
        let mut m = FuzzyMatcher::new();
        let exact = m.score("cl", "cl").expect("exact matches");
        let partial = m.score("cl", "clone").expect("prefix matches");
        assert!(exact > partial, "exact {exact} beats partial {partial}");
    }

    #[test]
    fn filter_ranks_and_preserves_tied_order() {
        let mut m = FuzzyMatcher::new();
        let items = ["alpha", "bravo", "abracadabra"];
        let out = m.filter(items, "a", |s| s);
        // Every item contains an `a`; ties keep input order.
        assert_eq!(out.len(), 3);
