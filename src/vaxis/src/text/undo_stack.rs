...
//!
//! The snapshot shape (`S`) is whatever state the caller needs to restore, for
//! example a `(String, usize)` for a single-line input or a richer struct for a
//! multi-line editor. The stack is agnostic to that shape.

/// A last-in-first-out stack of undo snapshots.
///
/// `push` stores a clone of the caller's state so subsequent mutations don't
/// invalidate the snapshot. `pop` returns the snapshot directly (without
/// re-cloning) because the stack no longer retains it.
#[derive(Debug)]
...
impl<S> UndoStack<S> {
    ...
    /// Push a snapshot onto the stack, taking ownership. Callers that still need
    /// access to the underlying state should clone before calling.
...
mod tests {
    ...
    fn works_with_tuple_snapshot_shapes() {
        // A tuple- or struct-shaped snapshot must round-trip unchanged. Verify a
        // (String, usize) compiles and comes back byte-for-byte.
