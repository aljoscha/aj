...
mod tests {
    ...
    use crate::log::{
        ...
        ThreadFilter, persistence_failure_channel_with_hook,
    ...
    #[tokio::test]
    async fn failed_forwarder_append_signals_once_after_fusing_the_shared_log() {
        let (_dir, log) = fresh_log();
        {
            let mut guard = log.lock().await;
        ...
        let after_report = Arc::new(move || {
            let _ = arrived_tx.send(());
            let _ = release_rx
                .lock()
                .expect("release hook mutex poisoned")
                .recv();
        });
        let (failure_signal, mut failures, draining) =
            persistence_failure_channel_with_hook(after_report);
        ...
        let invoked = Arc::clone(&listener);
        let invoked_event = event.clone();
        let worker = std::thread::spawn(move || {
            tokio::runtime::Builder::new_current_thread()
                .enable_all()
                .build()
                .expect("listener runtime")
                .block_on(invoked(&invoked_event))
                .is_err()
        });
        arrived_rx
            .recv_timeout(std::time::Duration::from_secs(1))
            .expect("listener reported its failure");
        let lock_was_held = log.try_lock().is_err();
        let reported = failures.try_recv();
        let _ = release_tx.send(());
        let first_failed = worker.join().expect("listener worker");

        assert!(first_failed, "first append fails");
        assert!(
            lock_was_held,
            "the signal was sent while the log mutex was held"
        );
        assert!(
            draining.load(std::sync::atomic::Ordering::Acquire),
            "the signal set the intake gate before releasing the log mutex"
        );
        let reported = reported.expect("first failure signal");
        let fused = log
            .lock()
            .await
            .write_failure()
        ...
        assert!(
            ...
            "the same first failure was signalled more than once"
        );
    }
