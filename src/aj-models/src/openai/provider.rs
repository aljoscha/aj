...
async fn run_stream_inner(
    ...
    let client = Client::new(base_url, credential.key);
...
/// what an absent account means. No pricing step is needed: usage is all
...
impl StreamState {
    ...
    fn finalize_or_truncate(mut self) -> AssistantMessageEvent {
        if self.saw_terminal() {
            ...
            // OpenAI sends usage after `finish_reason`, so this is a no-op
            // for its truncations. Keep the harvest defensive for compatible
            // endpoints that disclose usage earlier.
            self.seal();
