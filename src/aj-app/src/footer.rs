...
impl AgentFooters {
    ...
    /// reseeds the transcript. The checkpoint's usage reports summarizer spend,
    /// not retained context, so occupancy comes from the post-compaction estimate.
    /// A missing entry is left untouched (nothing to display against yet).
