...
/// or its chord is reserved. Terminal protocol support does not restrict overrides.
/// Beyond that, the accepted set must leave the final bindings collision-free:
/// the global chords stay mutually distinct, and an
...
mod tests {
    ...
    /// Overrides may use the full chord grammar, even when legacy terminal
    /// encodings cannot distinguish them or carry their modifiers.
    #[test]
    fn install_keybindings_accepts_protocol_dependent_chords() {
        let _guard = STORE_TEST_GUARD.lock().unwrap_or_else(|e| e.into_inner());
        let overrides = [
            ("aj.palette.open", "ctrl+shift+p"),
            ("aj.agent.open", "shift+enter"),
            ("aj.history.open", "f21"),
            ("aj.tools.expand", "ctrl+i"),
            ("aj.thinking.toggle", "alt+]"),
            ("aj.usage.reset", "super+k"),
        ];
        let problems = install_keybindings(
            overrides.map(|(action, chord)| (action.to_string(), chord.to_string())),
        );

        assert!(problems.is_empty(), "valid overrides install: {problems:?}");
        for (action, chord) in overrides {
            assert_eq!(effective_chord(action), Some(chord), "{action}");
        }
