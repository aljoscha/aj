...
- **Per-cell style snapshots.** `vt.cell(row, col)` returns an owned
  ...
  assertion runs. Several rendering tests (`markdown.rs`,
  `tui_render.rs`, `overlay_style_leak.rs`) lean on this so they don't
  each re-implement the parser reach-through.
...
`async_tui::wait_for_render(&mut tui).await` pumps `Tui::next_event`
until the throttled render fires, dispatching any input events to the
`Tui` along the way, then calls `tui.render()`. The sync-tier
`support::render_now` is a one-shot `tui.render()` for the
synchronous engine; the deliberate name asymmetry — `render_now()`
(no await) vs `wait_for_render().await` — makes the engine choice
obvious at the call site. See `tests/async_tui.rs` for examples.
