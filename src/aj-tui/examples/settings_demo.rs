...
fn editor_submenu_factory() -> SubmenuFactory {
    Box::new(move |current_value: &str, done: SubmenuDoneCallback| {
        ...
        let mut list = SelectList::new(
            items,
            7,
            // The tui crate is palette-agnostic by design (see PORTING.md
            // F33); this example assembles its own theme inline rather
            // than pulling from a missing upstream `Default`.
            SelectListTheme {
        ...
        let boxed: Box<dyn Component> = Box::new(list);
        boxed
    })
}
...
async fn main() {
    ...
    tui.add_child(Box::new(Text::new(
