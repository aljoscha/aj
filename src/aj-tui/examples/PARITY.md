...
- Slash-command autocomplete is wired through
  ...
  and `/clear`.
- Chat messages (both user and bot) render with `padding_y = 1`,
  matching the welcome banner's default and giving each turn a blank
  line above and below.
- Slash-command gates use indices, not raw `children.len()` arithmetic.
  `/delete` removes the message before the editor whenever
  `editor_index > 1` (i.e. at least one message exists); `/clear`
  walks every index from `editor_index - 1` down to `1` and removes
  it. The TS demo's gates (`children.length > 3` for delete,
  `splice(2, children.length - 3)` for clear) silently no-op when
  there is exactly one message because they assume a two-component
  welcome layout that the demo only renders as a single component;
  the index-based shape does the right thing for any message count.
- Submit-while-thinking is dropped one layer below this example: the
  editor honours `disable_submit = true` and never fires `on_submit`,
  so [`take_submitted`] returns `None` and `handle_submitted_text`
  bails before the slash-command match. There's no separate
  `is_responding` guard at the example level.
