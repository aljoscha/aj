...
The default `MarkdownTheme` seeds `hyperlinks` from
`aj_tui::capabilities::get_capabilities().hyperlinks`. Detection is
...
default off. Hosts that want to override the detection should either
mutate `theme.hyperlinks` directly or call
`aj_tui::capabilities::set_capabilities`.
...
- Async `#[tokio::main]` loop that selects over [`Tui::next_event`] and a
  ...
  when the editor is added to `tui.root`, so the popup repaints the
  moment results arrive — no per-frame polling.
...
- Loader is styled cyan (spinner) + dim (message) via
  `Loader::set_spinner_style` / `set_message_style`.
