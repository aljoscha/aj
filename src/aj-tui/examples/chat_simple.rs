        ...
        tokio::select! {
            ...
            _ = bot_ready => {
                if let BotState::Thinking { loader_ptr, .. } = bot_state {
                    ...
                    // padding_y = 1 puts a blank line above and below each
                    // chat message so the user can visually scan the
                    // turn-taking. Matches the spacing chat_simple uses
                    // throughout.
                    let msg = Markdown::new(response, 1, 1, markdown_theme(), None);
    ...
    if !trimmed.is_empty() {
        match trimmed.as_str() {
            ...
            _ if matches!(bot_state, BotState::Idle) => {
                // User message rendered as markdown verbatim. padding_y
                // matches the bot reply path so user and assistant
                // turns sit on the same vertical rhythm.
                let msg = Markdown::new(&text, 1, 1, markdown_theme(), None);
                let pos = editor_index(tui);
                tui.insert_child(pos, Box::new(msg));
