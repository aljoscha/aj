...
//!
//! Note on input: a `handleInput(data: string)` method that takes raw
//! terminal bytes is intentionally absent. crossterm's `EventStream`
//! parses bytes into typed events one layer down, so the per-component
//! input entry point is [`crate::component::Component::handle_input`]
//! — a typed [`crate::keys::InputEvent`] rather than a `&str`. Callers
//! that need to inject a paste send an
//! [`crate::keys::InputEvent::Paste`].
...
mod tests {
    ...
    fn box_dyn_editor_component_round_trip() {
        ...
        for mut ed in [editor, input] {
            ...
            // Cursor-relative insert: the `Editor` appends after the
            // existing text (cursor sits at end after `set_text`); the
            // `TextInput` inserts at byte offset 0 because `set_value`
            // clamps the cursor without moving it. Tolerate both
            // behaviors: assert only that the optional method doesn't
            // panic and produces *some* mutation.
            let before = ed.text();
            ed.insert_text_at_cursor("X");
            let after = ed.text();
            // `TextInput::insert_text_at_cursor` is the default no-op
            // because `TextInput` doesn't implement the optional method
            // (single-line, no cursor-relative-insert). Tolerate both
            // outcomes.
