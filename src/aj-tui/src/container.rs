...
/// required-at-construction shape: every component that needs a
/// handle holds it from birth, so the container has no forwarding
/// pass on `add_child` / `insert_child`.
...
impl Container {
    ...
    /// equality on the underlying component).
    ///
    ...
    /// currently in the container, so a missing child doesn't tear an
    /// unrelated item out of the list.
    ///
    /// The boxed component is returned (rather than discarded) so
    /// callers can re-attach it elsewhere. Callers that want to drop
    /// the removed child write `let _ = c.remove_child_by_ref(child);`.
...
mod tests {
    ...
    fn last_index_tracks_the_current_len() {
        ...
        // Detach via by-ref using the held heap address. (There is no
        // index-based shortcut; `remove_child_by_ref` is the only
        // detach API.)
    ...
    /// `remove_child_by_ref` removes the first child whose data pointer
    /// matches and returns the boxed component. Pins the happy path: a
    /// caller holding a borrow of an added child can detach it without
    ...
    /// Passing a `Component` reference that was never added to the
    /// container returns `None` and leaves the container untouched.
    #[test]
    ...
    /// Identity (not equality) is the contract: a distinct instance of
    /// the same type with byte-identical fields is treated as a
    /// different child and skipped.
    #[test]
