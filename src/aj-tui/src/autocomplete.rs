...
//! # Filesystem walking
//!
//! All filesystem walking goes through the [`ignore`] crate, which
//! honors `.gitignore`, `.ignore`, global gitignore, and hidden-file
//! rules out of the box. Hidden files are still surfaced via
//! `.hidden(false)`; the `.git` subtree is excluded by both the
//! per-walker `filter_entry` and a defense-in-depth
//! [`path_has_git_component`] check on every yielded path.
...
pub trait AutocompleteProvider: Send + Sync {
    ...
    /// Whether the editor should fire a file-completion request at
    /// this cursor position when the user explicitly asks for one
