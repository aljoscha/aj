...
//! tui.start()?;
//! // Components added to tui.root and any overlays ride along.
//! loop {
...
fn extract_cursor_position(lines: &mut [String], viewport_height: usize) -> Option<CursorPosition> {
    ...
    let start = lines.len().saturating_sub(viewport_height);
...
pub struct Tui {
    /// The root container holding base content.
    pub root: Container,
...
impl Tui {
    ...
    /// Components can be added to `tui.root` and [`RenderHandle`]s can
    /// be minted via [`Tui::handle`] before `start` is called.
    pub fn new(terminal: Box<dyn Terminal>) -> Self {
        let (render_tx, render_rx) = mpsc::unbounded_channel::<()>();
        let render_handle = RenderHandle {
            tx: render_tx.clone(),
        };
        let mut root = Container::new();
        // Give the root its handle up front so anything added via
        // `tui.root.add_child(...)` (the idiomatic entry point) picks
        // it up automatically.
        root.set_render_handle(render_handle);
        Self {
            root,
            terminal,
    ...
    pub fn show_overlay(
        ...
        // Wire the overlay into the same render-request channel the
        // root children use. The overlay can now ask for repaints via
        // its own async work without a separate setup step.
        component.set_render_handle(RenderHandle {
            tx: self.render_tx.clone(),
        });
    ...
    fn composite_overlays(&mut self, lines: &mut Vec<String>, width: usize, height: usize) {
        ...
        // wants `u16` to match the predicate signature — the narrowing
        // cast is safe because both come from `terminal.columns()` /
        // `terminal.rows()`, which return `u16` to begin with.
        let cols = width as u16;
        let rows = height as u16;
