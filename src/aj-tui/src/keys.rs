...
/// at all. `Parser::advance` clears its accumulator on `Err` from the
/// CSI parse, so they're consumed at the byte layer before reaching
/// us. We don't currently send `\x1b[16t` (no image support), so the
/// only source of these replies is environmental (e.g. tmux
/// passthrough), and we have no cell-dimension state to update from
/// them.
...
/// Enter-equivalent encoding that some terminals deliver as a byte
/// sequence the keybindings registry can't pattern-match.
///
...
/// these byte-form fallbacks are part of the newline branch and
/// bypass the submit branch's gate.
///
...
///   `Ctrl+J == LF`. Yes, this means a user pressing Ctrl+J (alone)
///   inserts a newline; we ship no other default binding for
///   Ctrl+J. Combined modifiers like `Ctrl+Shift+J` do *not* match
///   — those are distinct keystrokes.
...
/// any byte-form fallback. Ctrl+Enter has no byte-form fallback,
...
/// key being a single character, a named key (`enter` / `return`, `escape` /
/// `esc`, `tab`, `backspace`, `delete`, `insert`, `up`, `down`, `left`,
/// `right`, `home`, `end`, `pageUp` / `pageup`, `pageDown` / `pagedown`,
/// `space`), or a function key like `f1` through `f24`.
///
...
pub fn key_id_matches(event: &InputEvent, key_id: &str) -> bool {
    ...
    // Some terminals report `Shift+letter` as `Char('A')` rather than
    // `Char('a') + SHIFT`. Two flavours land on us:
    //
    // - **Legacy mode**: crossterm's `parse_utf8_char` runs every
    //   non-control byte through `char_code_to_event`, which sets
    //   SHIFT on uppercase ASCII letters. The keystroke `Shift+a`
    //   arrives as `Char('A') + SHIFT`.
    // - **Kitty alternate-keys (flag 4)**: when the terminal reports
    //   `\x1b[97:65;2u`, crossterm replaces the keycode with the
    //   shifted char *and clears the SHIFT bit*, so the same
    //   keystroke arrives as `Char('A') + NONE`.
    //
    // Descriptor `shift+a` accepts both shapes (plus `Char('a') +
    // SHIFT`); descriptor `a` accepts only `Char('a')` with no
    // effective shift. We fold the case only when the descriptor
    // explicitly mentions shift, so a bare `a` matches exclusively
    // a typed-lowercase 'a' and never an uppercase event.
    //
    // Crossterm also has a dedicated `KeyCode::BackTab` for the
    // legacy `\x1b[Z` sequence, which it always tags with
    // `KeyModifiers::SHIFT`. Normalize that to `KeyCode::Tab + SHIFT`
    // so a single `shift+tab` descriptor matches both the legacy
    // BackTab shape and the modern Kitty `Tab + SHIFT` shape.
    let (event_code, event_mods) = match key.code {
        KeyCode::Char(c) if c.is_ascii_uppercase() && parsed.shift_explicit => (
...
pub fn format_key_descriptor(event: &InputEvent) -> Option<String> {
    ...
    // Reject modifiers outside the descriptor grammar. Crossterm reports
    // HYPER and META under Kitty enhancement flags, but `parse_key_id`
    // only accepts `ctrl`, `alt`, `shift`, and `super`. Emitting a
    // descriptor that silently drops Hyper/Meta would be a one-way
    // string the matcher would reject on the way back, so bail.
    let supported_mods =
        KeyModifiers::SHIFT | KeyModifiers::CONTROL | KeyModifiers::ALT | KeyModifiers::SUPER;
    if !(key.modifiers - supported_mods).is_empty() {
        return None;
    }

    // Normalize `BackTab + SHIFT` into the `Tab + SHIFT` shape so the
    // emitted descriptor (`"shift+tab"`) round-trips through
    // `key_id_matches`, which already folds BackTab the same way.
    let (code, mut mods) = match key.code {
        KeyCode::BackTab => (KeyCode::Tab, key.modifiers | KeyModifiers::SHIFT),
        _ => (key.code, key.modifiers),
    };
...
/// Parse a descriptor string like `"ctrl+shift+p"` into the structured
/// form [`key_id_matches`] uses for comparison.
///
/// **Strict-rejection contract for unknown modifiers.** When the
/// descriptor contains a `+`-separated token that isn't `ctrl`, `alt`,
/// `shift`, or `super`, the whole descriptor is rejected (returns
/// `None`). The alternative — silently dropping unknown tokens so
/// `"hyper+a"` parses as `"a"` with no modifiers — is a bug factory:
/// a typo in a config key (`super` mistyped as `superr`, a stale
/// `meta` carried forward without renaming, etc.) silently changes
/// which keystrokes the binding fires on. Failing closed surfaces
/// typos at registration time and is pinned by
/// `unknown_modifier_names_reject_the_match` in `tests/keys.rs`.
...
mod key_id_tests {
    ...
    #[test]
    fn bare_letter_f_is_not_an_f_key_prefix() {
        // Regression: the F-key arm in `parse_key_id` accepted `"f"`
        // as an `f<digits>` prefix because `"".all(...)` is vacuously
        // true, then bailed out parsing the empty suffix — silently
        // breaking every descriptor whose key was `f` (notably the
        // editor's `ctrl+f` / `alt+f` defaults).
        assert!(key_id_matches(
    ...
    #[test]
    fn shift_letter_descriptor_accepts_three_event_shapes() {
        // The same physical `Shift+a` press arrives as one of three
        // shapes depending on terminal mode and crossterm's parser
        // path:
        //
        //   1. Legacy mode (most terminals without Kitty support):
        //      crossterm's `char_code_to_event` sets SHIFT on uppercase
        //      ASCII bytes, so we see `Char('A') + SHIFT`.
        //   2. Kitty disambiguate-only (flag 1): the terminal reports
        //      `\x1b[97;2u`, crossterm parses to `Char('a') + SHIFT`.
        //   3. Kitty alt-keys (flag 4): the terminal reports
        //      `\x1b[97:65;2u`, crossterm replaces the keycode with
        //      the shifted char and *clears* SHIFT, so we see
        //      `Char('A') + NONE`.
        //
        // The descriptor `"shift+a"` must accept all three shapes.
        assert!(key_id_matches(
            &key(KeyCode::Char('A'), KeyModifiers::SHIFT),
            "shift+a",
        ));
    ...
    #[test]
    fn no_shift_descriptor_does_not_match_uppercase_event() {
        // The corollary to `shift_letter_descriptor_accepts_three_event_shapes`:
        // an unmodified `"a"` descriptor must NOT match an uppercase
        // event, because the user did press shift.
        assert!(!key_id_matches(
            &key(KeyCode::Char('A'), KeyModifiers::SHIFT),
            "a",
        ));
    ...
    #[test]
    fn ctrl_shift_letter_accepts_uppercase_char_event() {
        // Same fold rule applies for combined modifiers. A Kitty-mode
        // `Ctrl+Shift+a` reported via alt-keys (`\x1b[97:65;6u`)
        // arrives as `Char('A') + CTRL` (SHIFT cleared by crossterm
        // when alt-keys is active). The descriptor `"ctrl+shift+a"`
        // must still match.
        assert!(key_id_matches(
            &key(
                KeyCode::Char('A'),
        ...
        // Without `shift+` in the descriptor, an uppercase event with
        // a different modifier doesn't match — the matcher won't
        // silently insert the shift.
        assert!(!key_id_matches(
            &key(KeyCode::Char('A'), KeyModifiers::CONTROL),
            "ctrl+a",
        ));
    ...
    #[test]
    fn format_descriptor_back_tab_round_trips_as_shift_tab() {
        // Crossterm decodes legacy `\x1b[Z` as `BackTab + SHIFT`. The
        // descriptor side normalizes that to the canonical `shift+tab`
        // string so a single binding works on both legacy and Kitty
        // terminals.
        let event = key(KeyCode::BackTab, KeyModifiers::SHIFT);
        assert_eq!(format_key_descriptor(&event).as_deref(), Some("shift+tab"),);
        assert!(key_id_matches(&event, "shift+tab"));
    }

    #[test]
    fn format_descriptor_returns_none_for_hyper_or_meta_modifiers() {
        // Crossterm reports HYPER / META under the Kitty
        // `REPORT_ALL_KEYS_AS_ESCAPE_CODES` enhancement flag, but the
        // descriptor grammar only spells `ctrl`, `alt`, `shift`, and
        // `super`. Emitting a string that silently dropped Hyper/Meta
        // would produce a one-way descriptor (formatting → string,
        // string → matcher would falsely reject the original event).
        // Bail instead.
        assert!(format_key_descriptor(&key(KeyCode::Char('a'), KeyModifiers::HYPER)).is_none());
        assert!(format_key_descriptor(&key(KeyCode::Char('a'), KeyModifiers::META)).is_none());
        assert!(
            format_key_descriptor(&key(
                KeyCode::Char('a'),
    ...
    fn format_descriptor_round_trips_through_key_id_matches() {
        ...
        let cases = [
            ...
            key(KeyCode::Char('+'), KeyModifiers::CONTROL),
            // Super-modified Kitty bindings (`super+k`, `ctrl+super+k`,
            // and the four-modifier corner case).
    ...
    #[test]
    fn is_newline_event_recognizes_byte_form_fallbacks() {
    ...
    fn is_newline_event_excludes_keys_that_are_or_might_be_user_bindings() {
        ...
        // Ctrl+Enter has no byte-form fallback. Some users rebind
        ...
        // Combined modifiers — none have byte-form fallbacks.
        assert!(!is_newline_event(&key(
            KeyCode::Enter,
