...
/// Whether `seg` should be classified as whitespace for the purposes of
/// recording a wrap opportunity. Differs from
/// [`crate::ansi::is_whitespace_grapheme`]'s any-scalar rule in that a
/// multi-grapheme atomic segment counts as whitespace only when *every*
/// scalar is whitespace.
///
/// Without this stricter rule, an atomic segment like a paste marker
/// (`[paste #1 +20 lines]`, which contains literal spaces) would be
/// classified as whitespace and the wrapper would drop wrap
/// opportunities both before and after it: the trailing space of the
/// preceding word would see a "whitespace" next-segment and skip
/// recording the break point at the marker, and a non-whitespace word
/// directly following the marker would record an opportunity at the
/// wrong column. Using "every scalar" semantics treats single-grapheme
/// segments identically (single whitespace char vs. single
/// non-whitespace char) and only changes behavior for multi-grapheme
/// atomic units the caller explicitly passed in.
...
pub fn word_wrap_line(line: &str, max_width: usize) -> Vec<TextChunk> {
    ...
    // A single grapheme cluster cannot be split further (a 2-column
    // CJK char or a wide emoji is still one grapheme). Emit it whole
    // even though it overflows `max_width` — splitting bytes inside
    // a grapheme would corrupt it. This also serves as the recursion
    // base case for `wrap_segments`'s oversized-segment path.
    if segments.len() <= 1 {
        return vec![TextChunk::from_slice(line, 0, line.len())];
    }
...
mod tests {
    ...
    #[test]
    fn single_grapheme_wider_than_max_width_returns_it_whole() {
        // A 2-column CJK char with max_width = 1 cannot be split further
        // — splitting between the bytes of "你" would corrupt the grapheme.
        // Without the single-grapheme base case in `word_wrap_line` the
        // oversized-segment path in `wrap_segments` recursed on itself
        // forever and stack-overflowed.
        let chunks = word_wrap_line("你", 1);
        assert_eq!(
            chunks,
            vec![TextChunk {
                text: "你".into(),
        ...
        let chunks = word_wrap_line("\u{1F600}", 1); // 😀, width 2
        assert_eq!(
            chunks,
            vec![TextChunk {
                text: "\u{1F600}".into(),
                start_index: 0,
                end_index: "\u{1F600}".len(),
            }],
        );
    }
}
