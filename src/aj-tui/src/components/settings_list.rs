...
impl Component for SettingsList {
    ...
    fn handle_input(&mut self, event: &InputEvent) -> bool {
        ...
        // today: every branch that mutates `value` returns `true`,
        // and every branch that returns `false` leaves `value`
        // untouched (Ctrl/Alt-modified Key events that didn't match
        // any registry binding and aren't plain printables — e.g. a
        // `Ctrl+Z` under default bindings). A `if handled` gate
        // would therefore be functionally correct today, but would
        // be structurally fragile: a future TextInput change that
        // adds a return-false-but-mutate path would silently break
        // filter updates. Dropping the gate eliminates that drift
        // surface.
        //
