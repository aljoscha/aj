...
impl SettingsList {
    pub fn new(
        ...
        let search = options.enable_search.then(|| TextInput::new("> "));
        Self {
            items,
            filtered,
            theme,
...
impl Component for SettingsList {
    ...
    /// Forward invalidation to the active submenu so its caches clear
    /// when the framework signals a global render-affecting change
    /// (terminal resize, theme palette swap). The list's own renderer
    /// holds no per-frame cache, so there is nothing to clear here
    /// when no submenu is active.
    ...
    fn handle_input(&mut self, event: &InputEvent) -> bool {
        ...
        //
        // Filter recompute fires unconditionally after forwarding to
        // the search input. Auditing [`TextInput::handle_input`]
        // (PORTING.md F37): every branch that mutates `value` returns
        // `true`, and every branch that returns `false` leaves
        // `value` untouched (Ctrl/Alt-modified Key events that didn't
        // match any registry binding and aren't plain printables —
        // e.g. a `Ctrl+Z` under default bindings). A `if handled`
        // gate would therefore be functionally correct today, but
        // would be structurally fragile: a future TextInput change
        // that adds a return-false-but-mutate path would silently
        // break filter updates. Dropping the gate eliminates that
        // drift surface.
        //
