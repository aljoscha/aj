...
/// the divergence would require a stateful scan that tracks whether
/// we're inside an unclosed `"`. Documented in `PORTING.md`; no
/// caller currently exercises the quoted-typing path.
...
mod tests {
    ...
    /// Pin down the slash-command auto-trigger alphabet (F18). We
    /// gate the "keep refining the popup as the user types" branch
    /// on the regex `[a-zA-Z0-9.\-_]` applied to the just-inserted
    /// character; [`is_identifier_char`] is the byte-level equivalent.
    /// A future tweak that, say, adds `/` or drops `_` would silently
    /// change which keystrokes auto-trigger completion inside a slash
    /// or `@` context — this test fails loudly when that alphabet
    /// drifts.
    #[test]
    ...
    fn ends_in_symbol_context_recognises_at_and_hash_at_token_boundary() {
        ...
        // PORTING.md divergence; we only check the closed form here).
        assert!(!ends_in_symbol_context("@\"foo bar\""));
    }
}
