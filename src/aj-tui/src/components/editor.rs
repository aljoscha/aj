...
/// Whether `before` (the text immediately before the cursor on the
/// current line) ends in an `@`-attachment or `#`-symbol token at a
/// token boundary. Equivalent to the regex `(?:^|[\s])[@#][^\s]*$`.
///
/// Used by:
/// 1. [`Editor::is_in_symbol_context`] — gates the auto-trigger /
///    re-trigger paths after typing or deletion.
/// 2. [`Editor::autocomplete_debounce_for`] — decides whether the
///    20ms attachment-debounce window applies. Without this branch
///    covering `#`, rapid typing inside a `#1234`-style symbol
///    reference would call the provider on every keystroke instead
///    of coalescing into one call.
///
/// **Open-quoted attachment**: an in-progress quoted attachment of
/// the form `@"<chars-with-spaces>` (no closing quote yet) is
/// rejected by this predicate as soon as the space lands. Closing
/// the gap would require a stateful scan that tracks whether
/// we're inside an unclosed `"`; no caller currently exercises
/// the quoted-typing path, so the simpler regex-shaped check
/// stays.
fn ends_in_symbol_context(before: &str) -> bool {
    // Pick whichever symbol appears latest in the prefix. If both
    // exist (e.g. `@a #b`), the rightmost wins, matching the
    // single-symbol regex.
...
/// Decode Kitty CSI-u Ctrl+<letter> sequences embedded in pasted text
/// back to the literal control byte.
///
/// Some terminals (notably tmux popups configured with
/// `extended-keys-format=csi-u`) re-encode control bytes inside a
/// bracketed paste as `ESC [ <codepoint> ; 5 u`. The classic example
/// is `ESC [ 106 ; 5 u` (Ctrl+J = newline) appearing between every
/// pasted line. Without this decode pass, the editor's per-char
/// control-byte filter would strip the `ESC` and leak the printable
/// tail (`[106;5u`) into the buffer, garbling the paste.
///
/// Decodes `\x1b\[(\d+);5u` sequences. Only Ctrl+<ASCII-letter>
/// codepoints (97..=122 and 65..=90) are decoded; other modifier-5
/// sequences are left intact so the filter handles them as before.
///
/// Hand-rolled rather than pulling in `regex` as a runtime dep: the
/// pattern is fixed and the only structure is a digit run between two
/// constant byte sequences.
fn decode_csi_u_ctrl_letters(text: &str) -> String {
    // Fast path: no ESC, nothing to do. Avoids allocating a new
    // `String` for every paste of normal text.
    ...
    let mut i = 0;
    while i < bytes.len() {
        // Only step the scanner one byte at a time when we're on an
        // ASCII byte that's not the start of a CSI-u sequence. For
        // non-ASCII (multi-byte UTF-8) graphemes we copy the entire
        // codepoint through using `text[i..]`'s `chars()` iterator so
        // we never split a UTF-8 sequence.
        if !bytes[i].is_ascii() {
            // Length of the next char in bytes.
        ...
        // Parse a digit run starting at `i + 2`.
        let digits_start = i + 2;
        let mut digits_end = digits_start;
        while digits_end < bytes.len() && bytes[digits_end].is_ascii_digit() {
            digits_end += 1;
        }
        // Require at least one digit followed by `;5u`. Anything else
        // is left untouched (e.g. cursor-movement sequences).
        ...
        let decoded = match code {
            // Lowercase a..z → Ctrl+a..Ctrl+z (\x01..\x1a)
            97..=122 => char::from_u32(code - 96),
            // Uppercase A..Z → Ctrl+A..Ctrl+Z (\x01..\x1a)
        ...
        match decoded {
            Some(c) => {
                out.push(c);
                i = digits_end + 3; // consume `;5u`
            }
            None => {
                // Out of range — leave the literal `ESC[<digits>;5u`
                // bytes in place so the existing filter handles them.
                out.push(char::from(bytes[i]));
                i += 1;
            }
        }
...
pub struct Editor {
    ...
    /// Map of paste id → the literal content the marker stands in
...
impl Editor {
    ...
    ///
    /// The theme is required: the tui crate stays palette-agnostic;
    /// build the theme from the agent's central palette (or a test
    ...
    ///
    /// Input is normalized via [`Editor::normalize_text`] so callers
    /// don't have to: `\r\n` and lone `\r` collapse to `\n`, and any
    /// `\t` characters expand to four spaces. The editor's internal
    /// model is whitespace-only — multi-line text is one `String` per
    /// logical line, no embedded `\r` or `\t`.
    ...
    pub fn max_visible_lines(&self) -> usize {
        ...
        // `max(5, floor(rows * 0.3))`. The `floor` falls naturally
        // out of integer multiply-then-divide.
        ((rows * 3) / 10).max(5)
    }
    ...
    /// Input is normalized via [`Editor::normalize_text`]: `\r\n` and
    /// lone `\r` collapse to `\n`, and `\t` expands to four spaces.
    /// Other control characters are stripped (a pasted `\0` or `\x01`
    /// shouldn't land in the document).
    ///
    ...
    /// Normalize text for storage in the editor:
    ///
    /// - Line endings: `\r\n` and lone `\r` collapse to `\n`.
    /// - Tabs: every `\t` expands to four spaces.
    ///
    /// Centralizing these rules in one helper means
    /// [`Editor::set_text`], [`Editor::insert_text_at_cursor`], and the
    /// paste path agree exactly on what counts as "input the editor
    /// stores literally" versus "ambiguous whitespace we have to pick
    /// a representation for". The editor's internal model never holds
    /// embedded `\r` or `\t`.
    ...
    fn submit_value(&mut self) {
        ...
        // pasted content, not the marker placeholder. Trim leading
        // and trailing whitespace so the consumer sees the typed
        // message without spurious empty padding lines.
        let text = self.get_expanded_text().trim().to_string();
    ...
    fn handle_paste(&mut self, text: &str) {
        ...
        self.cancel_autocomplete();
        self.save_undo();
        // Some terminals (notably tmux popups with
        // `extended-keys-format=csi-u`) re-encode control bytes inside a
        // bracketed paste as Kitty CSI-u Ctrl+<letter> sequences
        // (`ESC [ <codepoint> ; 5 u`). Decode those back to their
        // literal byte before the per-char filter below would strip the
        // ESC and leak the printable tail (e.g. `[106;5u` for a
        // re-encoded newline) into the editor.
        let decoded = decode_csi_u_ctrl_letters(text);
        // Normalize line endings and tabs through the shared helper so
        // paste, `set_text`, and `insert_text_at_cursor` all agree on
        // the input shape stored in the editor's line buffer.
        let normalized = Self::normalize_text(&decoded);
        // Filter out control characters (newlines and the four-space
        // tab expansion are already in the normalized text).
    ...
    /// the literal content it stands in for.
    ///
    ...
    fn dispatch_autocomplete_request(&mut self, mode: AutocompleteMode, auto_apply_single: bool) {
        ...
        // Only the `@`-attachment and `#`-symbol paths get a debounce
        // — Tab and `/` command autocomplete fire immediately. See
    ...
    /// Called right after inserting `@` or `#`: returns true iff the
    /// character before the just-inserted symbol is whitespace or the
    /// symbol is at the start of the line. Equivalent to the regex
    /// `(?:^|[ \t])[@#]$` guard: `@foo` or `#bar` in the middle of a
    /// word shouldn't open the fuzzy-file / symbol popup.
    ...
    /// whitespace appears between that symbol and the cursor.
    /// Equivalent to the regex `(?:^|[\s])[@#][^\s]*$` applied to the
    /// text before the cursor. Implementation in
...
impl crate::editor_component::EditorComponent for Editor {
    ...
    fn set_autocomplete_provider(&mut self, provider: std::sync::Arc<dyn AutocompleteProvider>) {
    ...
    fn border_color(&self) -> Option<std::sync::Arc<dyn Fn(&str) -> String>> {
        // The theme is the source of truth for the border-color
        // closure; expose it directly so hosts can read the current
        // value (matching the read-and-write contract on the trait).
        Some(std::sync::Arc::clone(&self.theme.border_color))
    }

    fn set_border_color(&mut self, color: std::sync::Arc<dyn Fn(&str) -> String>) {
        // Mutating the theme would be cleaner if `set_theme` existed,
        // but a single-axis swap is what the trait advertises and is
        // also what host UI-mode toggles need (border tint changes
        // independently of selectable colors). Drop the render cache
        // so the next paint picks up the new color.
    ...
    fn handle_input(&mut self, event: &InputEvent) -> bool {
        ...
        match event {
            ...
            InputEvent::Key(key) => {
                ...
                //   etc.). Jump-mode does not consult
                //   `tui.select.cancel`. Control characters drop
                //   through so the parent surface can handle
                //   Esc / Ctrl+C / etc. itself.
                ...
                // Ctrl+J). Checked before submit so Shift+Enter inside
                // a submit-enabled editor produces a newline instead
                // of a submit.
                //
                // The byte-form fallback fires regardless of
                // `disable_submit` — `\x1b\r`, raw LF, and
                // `\x1b[13;2~` are always treated as newline. There
                // is no "submit-key becomes newline under
                // `disable_submit`" arm here: when `disable_submit`
                // is set, the submit key is silently consumed in the
                // submit branch below.
                ...
                //
                // When `disable_submit` is true, the submit key is
                // silently consumed: the handler returns `true` (key
                // consumed, no fall-through) without doing anything
                // else.
...
mod tests {
    ...
    /// `Editor::new` takes the theme as a required argument and
    /// applies it directly. The render path picks up the supplied
    /// `border_color` immediately — no `set_theme` call required.
    #[test]
    ...
    fn test_editor_newline() {
        // Shift+Enter (the default `tui.input.newLine`) inserts a
        // newline. Plain Enter under `disable_submit = true` is a
        // silent no-op.
        let mut editor = Editor::new(RenderHandle::detached(), identity_theme());
        editor.set_focused(true);
    ...
    fn is_identifier_char_matches_expected_alphabet() {
        ...
        // The three non-alphanumeric extras in the trigger regex.
        for c in ['.', '-', '_'] {
        ...
        // punctuation/symbols, and non-ASCII letters (the trigger
        // regex is ASCII-only; any change to broaden it is a
        // behavior change worth a deliberate test update).
        for c in [
            ' ', '\t', '\n', '/', '@', ',', ';', ':', '(', ')', '[', ']', '{', '}', '+', '=', '*',
            '?', '!', '~', '#', '$', '%', '^', '&', '<', '>', '|', '\\', '"', '\'', '`', 'é', 'ü',
            'я', '中',
    ...
    /// rejects: a sigil in the middle of a word (`foo@bar`), and
    /// any whitespace between the sigil and the cursor (`@foo bar`).
    /// A drift here silently disables debouncing for `#`-symbol
    /// triggers (the gap closed by the helper).
    #[test]
    fn ends_in_symbol_context_recognises_at_and_hash_at_token_boundary() {
        // Positive: bare `@` / `#` at start of buffer.
        assert!(ends_in_symbol_context("@"));
        assert!(ends_in_symbol_context("#"));
        // Positive: open token after start.
        assert!(ends_in_symbol_context("@foo"));
        assert!(ends_in_symbol_context("#1234"));
        assert!(ends_in_symbol_context("@a"));
        // Positive: prefixed by whitespace.
        assert!(ends_in_symbol_context("hello @file.rs"));
        assert!(ends_in_symbol_context("hello\t#1234"));
        assert!(ends_in_symbol_context("hello @"));
        // Positive: rightmost symbol wins when the prefix ends in
        // either form.
        assert!(ends_in_symbol_context("@a #b"));
        assert!(ends_in_symbol_context("#a @b"));

        // Negative: no symbol at all.
        assert!(!ends_in_symbol_context(""));
        assert!(!ends_in_symbol_context("hello world"));
        // Negative: symbol in the middle of a word — no whitespace
        // boundary before it.
        assert!(!ends_in_symbol_context("foo@bar"));
        assert!(!ends_in_symbol_context("foo#bar"));
        assert!(!ends_in_symbol_context("user@example.com"));
        // Negative: whitespace between symbol and cursor.
        assert!(!ends_in_symbol_context("@foo bar"));
        assert!(!ends_in_symbol_context("#1234 "));
        // Negative: closed-quoted attachment exits the unquoted
        // branch. The open-quoted form `@"foo` is the documented
