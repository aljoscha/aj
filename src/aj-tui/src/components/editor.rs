...
impl Default for EditorTheme {
    fn default() -> Self {
        Self {
            border_color: Box::new(|s| format!("\x1b[90m{}\x1b[0m", s)),
...
pub struct Editor {
    ...
    /// Optional render-request handle the worker task uses to wake the
    /// driver's event loop when results arrive. Without this, the
    /// driver will only repaint on the next input event, so production
    /// code should always install one via [`Editor::set_render_handle`].
    autocomplete_render_handle: Option<RenderHandle>,
...
impl Editor {
    ...
    pub fn new() -> Self {
        ...
        Self {
            ...
            autocomplete_rx,
            autocomplete_tx,
            autocomplete_render_handle: None,
            history: Vec::new(),
            history_index: None,
            paste_buffer: None,
            pastes: HashMap::new(),
    ...
    /// Set the maximum number of visible lines before scrolling.
    ...
    ///
    /// Word segmentation follows a three-class model: whitespace,
    /// punctuation (the ASCII punctuation set `(){}[]<>.,;:'"!?+\-=*/\\|&%^$#@~``), and
    /// "word" (everything else — letters, digits, most non-Latin scripts,
    /// emoji). Walking backward: first skip any trailing whitespace, then
    /// skip a run of graphemes in whichever of the remaining two classes
    /// the last grapheme belongs to. This matches the standard readline /
    /// Emacs behavior where `foo bar...` breaks into three words: `foo`,
    /// `bar`, and `...`.
    ...
    ///
    /// Uses the same three-class model as [`Editor::word_boundary_left`]:
    /// skip leading whitespace, then skip a run of the class of the first
    /// remaining grapheme.
    fn word_boundary_right(&self) -> usize {
        ...
        while i < graphemes.len() && is_whitespace_grapheme(graphemes[i].1) {
            new_col += graphemes[i].1.len();
            i += 1;
        }

        // Paste marker: if the non-whitespace run starts with a marker,
        // skip the whole marker atomically and return. Without this, the
        // `[` would look like punctuation and stop after one grapheme.
        if let Some((_start, end)) = marker_starting_at(line, new_col, &self.pastes) {
            return end;
        }

        if i < graphemes.len() {
            let first = graphemes[i].1;
            if is_punctuation_grapheme(first) {
                while i < graphemes.len() && is_punctuation_grapheme(graphemes[i].1) {
                    new_col += graphemes[i].1.len();
                    i += 1;
                }
            } else {
    ...
    fn handle_paste(&mut self, text: &str) {
        ...
        let normalized = text.replace("\r\n", "\n").replace('\r', "\n");
        // Filter out control characters (except newline and tab, which
        // we translate to space).
    ...
    fn adjust_scroll(&mut self, _visible_lines: usize) {
        ...
        let max_vis = self.max_visible_lines.max(5);
    ...
    fn apply_autocomplete_delivery(&mut self, delivery: AutocompleteDelivery) {
        ...
        list.set_theme(Self::clone_theme(&self.theme.select_list));
    ...
    /// Clone the SelectList theme fields. The theme stores `Box<dyn Fn>`
    /// closures, which aren't `Clone`; for our purposes the popup
    /// reuses the editor's configured theme, so we rebuild a default
    /// theme — callers that want a custom select-list theme can set
    /// it via `set_theme` and it will be applied via this path too.
    ///
    /// TODO: once `SelectListTheme` grows a `Clone` impl or a cheap
    /// handle, replace this with a direct clone.
    fn clone_theme(_source: &SelectListTheme) -> SelectListTheme {
...
impl Component for Editor {
    ...
    fn set_render_handle(&mut self, handle: RenderHandle) {
        self.autocomplete_render_handle = Some(handle);
    }
            ...
            if vl.has_cursor && self.focused {
                ...
                for g in &graphemes {
                    ...
                    if vis_col == col && !cursor_placed {
                        ...
                        new_line.push_str(&format!("\x1b[7m{}\x1b[27m", g));
                        cursor_placed = true;
                    } else {
                        new_line.push_str(g);
                    }
                ...
                if !cursor_placed {
                    ...
                    new_line.push_str("\x1b[7m \x1b[27m");
    ...
    fn handle_input(&mut self, event: &InputEvent) -> bool {
        ...
        match event {
            ...
            InputEvent::Key(key) => {
                ...
                // Character-jump mode intercept. Once set by Ctrl+] or
                // Ctrl+Alt+], the next input either:
                // - cancels the mode (a second Ctrl+]/Ctrl+Alt+] matching
                //   the direction, or Esc — both consumed by the editor),
                // - performs the jump (any other printable char), or
                // - cancels silently and falls through (non-printable
                //   input like arrow keys or Shift alone).
                ...
                // Newline: explicit `tui.input.newLine` (Shift+Enter)
                // or implicit (Enter when submit is disabled). Checked
                // before submit so Shift+Enter inside a submit-enabled
                // editor produces a newline instead of a submit.
                let is_newline = kb.matches(event, "tui.input.newLine")
                    || (matches!(key.code, KeyCode::Enter) && self.disable_submit);
                if is_newline {
                    drop(kb);
                    self.save_undo();
                    self.insert_newline_internal();
                ...
                if kb.matches(event, "tui.input.submit") && !self.disable_submit {
                    drop(kb);
                    ...
                    // Expand paste markers so the submitted value is
                    // the literal pasted content, not the placeholder.
                    let text = self.get_expanded_text();
                    self.submitted_text = Some(text.clone());
                    if let Some(ref mut on_submit) = self.on_submit {
                        on_submit(&text);
                    }
                    // Reset the editor to an empty document. Submit is
                    // a hard break: the previous content has been
                    // consumed, so the next keystroke should start a
                    // fresh editing session with a clean undo stack,
                    // no kill/yank/type-word chain, and the cursor
                    // at (0, 0).
