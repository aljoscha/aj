...
/// [`Component::render`], so a triggered render keeps the animation
/// up to date.
///
/// Layout: the spinner+message body is fed into an embedded [`Text`]
/// with `padding_x = 1, padding_y = 0`, and the rendered output is a
/// leading blank row plus the wrapped Text rows. So the spinner sits
/// one space in from the left, each row is right-padded to the
/// terminal width, and a long message wraps at `width - 2` instead
/// of overflowing.
///
...
/// # Construction
///
/// [`Loader::new`] requires spinner and message style closures up
/// front. Tests and other callers that don't care about styling can
/// use [`Loader::with_identity_styles`], which fills in identity
/// closures.
pub struct Loader {
    message: String,
    ...
    /// `padding_x = 1, padding_y = 0`. Reused across renders so the
    /// Text-internal cache (keyed on `(text, width)`) works for
...
impl Loader {
    ...
    ///
    /// The styling closures are required at construction. Use
    /// [`Loader::with_identity_styles`] for the common test/no-styling
    /// case to avoid the per-site `Box::new(|s| s.to_string())`
    /// boilerplate.
    ///
    ...
    pub fn new(
        ...
        // padding_x = 1, padding_y = 0 so we don't sandwich the
        // spinner row in blank rows on top of our own leading blank.
        ...
        // A freshly-constructed loader synchronously asks for a
        // render so its first frame appears even on an idle Tui
        // that has `set_initial_render(false)`.
    ...
    pub fn set_message(&mut self, message: &str) {
        ...
        // A message change must paint immediately, even on a static
        // or empty-frame loader where no animation pump exists to
        // coalesce the change into the next tick.
    ...
    pub fn start(&mut self) {
        ...
        // Without this, a `start` on an idle Tui with
        // `initial_render = false` and a static indicator wouldn't
        // paint until the next input arrives.
        self.request_repaint();
    }
    ...
    /// braille spinner. The timer resets so a swap mid-flight doesn't
    /// jump to an arbitrary frame of the new sequence, and the loader
    /// is implicitly (re)started — calling `set_indicator` after
    /// [`Loader::stop`] resumes animation, matching the contract that
    /// "I'm telling you which indicator to show" implies "and please
    /// show it now".
    ///
    /// Semantics:
    ///
    ...
    pub fn set_indicator(&mut self, options: Option<LoaderIndicatorOptions>) {
        ...
        // Reuse `start` so a stopped loader is reactivated, the timer
        // resets, the pump is (re)evaluated against the new frame
        // count, and a synchronous repaint is requested. The new
        // indicator surfaces on the next paint regardless of whether
        // it animates — so a swap to a static or empty indicator
        // doesn't wait for an unrelated render trigger.
        self.start();
    }
    ...
    ///
    /// Any state change that affects the visible loader (message
    /// swap, indicator swap, style swap, construction, start) calls
    /// this to keep the screen in sync without waiting for the next
    /// animation tick.
    ///
...
impl Component for Loader {
    ...
    fn render(&mut self, width: usize) -> Vec<Line> {
        ...
        // Compose the body: `${frame} ${msg}` when there's a frame,
        // else just the message (no leading separator space).
        // Verbatim frames bypass `spinner_style` so the caller's
        // pre-baked ANSI escapes aren't double-wrapped.
    ...
    /// Forward invalidation to the embedded [`Text`] body so its
    /// per-(text, width) render cache clears. The framework's
    /// `Tui::invalidate` (and any container ancestor) calls this on a
    /// global event that can change rendered output independently of
    /// the loader's own state — terminal resize, theme palette swap,
    /// capability re-detection. Without forwarding, the next
    /// `body.render(width)` would return the stale cached lines until
    /// `set_text` was called with a different value (which only
    /// happens when the message or active frame changes), even though
    /// the framework explicitly asked every cached output to be
    /// recomputed.
