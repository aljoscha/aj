...
impl Text {
    ...
    /// build a new `Text`. The optional background function is set
    /// separately via [`set_bg_fn`](Self::set_bg_fn).
...
impl Default for Text {
    /// Default constructs with empty text and `padding_x = padding_y = 1`.
    fn default() -> Self {
...
impl Component for Text {
    ...
    fn render(&mut self, width: usize) -> Vec<String> {
        ...
        // No content to render: empty and whitespace-only text both
        // mean "nothing to draw" — no top/bottom padding rows are
        // emitted.
        ...
        // Content lines: `left_margin + line + right_margin`, then pad
        // with spaces to reach the full terminal width. The right
        // margin and the trailing pad are applied even in the no-bg
        // case so every content row spans the full render width.
        ...
        // Top / bottom padding rows. The blank line is built once and
        // reused; in the bg case it routes through
        // `apply_background_to_line` (no-op padding pass since the
        ...
        // Cache the pre-fallback `result`. The tail fallback below only
        // fires on the *current* render; the cache stores the original
        // (possibly empty) `result` so a subsequent render with the
        // same args returns the cached vec verbatim. The fallback is
        // defensive against a hypothetical wrap output of zero lines;
