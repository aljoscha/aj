...
impl TextBox {
    /// Construct a `TextBox` with the given padding.
    ///
    /// Padding is fixed at construction; a caller that wants to swap
    /// padding mid-life should build a new `TextBox` and re-attach
    /// its children. The optional background closure is configured
    /// separately via [`TextBox::set_bg_fn`].
    ...
    /// Remove the first child whose identity matches `child`.
    ///
    ...
    /// currently in the box. The cache is only dropped when an actual
    /// removal happened — a miss is a no-op.
    ///
    /// The removed component is returned so callers can re-attach it
    /// elsewhere; discard with `let _ = tb.remove_child_by_ref(child);`.
...
impl Default for TextBox {
    /// Default padding of one cell in each axis.
    fn default() -> Self {
        Self::new(1, 1)
    }
}
...
impl Component for TextBox {
    ...
    fn render(&mut self, width: usize) -> Vec<String> {
        if self.children.is_empty() {
            ...
            // `add_child` / `remove_child` / `clear` already invalidate,
            // so a subsequent render with children re-installed comes
            // from those mutators rather than this branch.
            return Vec::new();
        }

        // Clamp content width to at least one cell. A degenerate render
        // width (`width = 0`, or `width < 2 * padding_x`) still produces
        // one cell of content rather than collapsing the whole box to
        // zero rows.
        ...
        if child_lines.is_empty() {
            // Children rendered no lines; leave the cache intact and
            // return empty so the caller skips this box entirely.
            return Vec::new();
        }
