//! Integration tests for the [`Text`] component, covering default
//! padding, tab normalization, empty/whitespace handling, the
//! non-empty-but-zero-rows fallback row, the degenerate-width clamp,
...
/// `Text::default()` should produce one blank row above the content
/// and one below (`padding_y = 1`). Constructing with
/// `Text::new(text, 1, 1)` resolves to the same render.
#[test]
...
/// Every `\t` is replaced with three spaces before wrapping. The
...
/// Empty input produces no rows at all — not even padding rows.
#[test]
...
/// Whitespace-only input is treated as empty so a Text whose content
/// was reset to spaces or newlines doesn't render visible padding
/// rows.
#[test]
...
/// `text.trim()` so it bypasses the empty-text branch.
/// `wrap_text_with_ansi` returns one line carrying the (invisible)
/// ANSI codes; the Text component then surrounds it with margins and
/// pads to full width. Visible width is the full render width — the
/// ANSI codes contribute zero visible cells, the rest is padding
/// spaces.
///
/// The `result.is_empty()` tail fallback is defensive against a
/// hypothetical empty wrap output, which neither this code nor any
/// real input actually reaches. Locked in here so that a future wrap
/// refactor that *does* return empty surfaces visibly rather than
/// silently.
#[test]
...
/// `content_width` is clamped to `max(1, width - 2 * padding_x)`, so
/// a width that's smaller than the horizontal padding still produces
/// at least one column of content rather than dropping the row.
#[test]
fn degenerate_width_smaller_than_horizontal_padding_still_renders_content() {
    let mut t = Text::new("hi", 5, 0);

    // width=4 is less than 2 * padding_x = 10. content_width is
    // clamped to 1, so "hi" wraps to two rows of one column each. We
    // don't pin the exact wrapped layout (that's wrap_text_with_ansi's
    // contract), we just assert the row didn't get dropped.
...
/// Each content row is composed as `left_margin + line + right_margin`
/// then padded with spaces to reach the full render width. Locks in
/// that every content row spans the full terminal width even in the
/// no-bg case.
#[test]
