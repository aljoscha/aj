...
#[test]
fn fitting_paste_marker_with_internal_space_keeps_word_break_after_it() {
    // Regression: a paste marker like `[paste #1 +20 lines]` contains
    // literal spaces in its text. If the wrapper used a permissive
    // "any scalar is whitespace" rule for atomic segments, the marker
    // would be classified as whitespace and the trailing-word break
    // immediately after it would be lost — overflow at the next word
    // would either backtrack to a stale earlier opportunity or
    // force-break mid-word. With segment-level "all-scalar" semantics
    // the marker is non-whitespace, the space after it records a
    // proper opportunity, and overflow at "trailing" backtracks
    // cleanly.
    let line = format!("abcdefghij {MARKER_1}trailing");
    let mut segments = vec![];
    let prefix: [&str; 10] = ["a", "b", "c", "d", "e", "f", "g", "h", "i", "j"];
    ...
    let suffix: [&str; 8] = ["t", "r", "a", "i", "l", "i", "n", "g"];
    for (i, g) in suffix.iter().enumerate() {
        segments.push(TextSegment {
            text: g,
            start_index: 11 + MARKER_1.len() + i,
        });
    }

    // max_width 35: "abcdefghij " (11) + MARKER_1 (20) + "trailing"
    // (8) = 39 visible columns. With the marker classified as
    // non-whitespace the only wrap opportunity sits between the
    // leading space and the marker, so the line splits there.
    let chunks = word_wrap_line_with_segments(&line, 35, &segments);
    assert_all_fit(&chunks, 35);
    assert_eq!(reconstruct(&line, &chunks), line);
    assert_eq!(
        chunks.iter().map(|c| c.text.as_str()).collect::<Vec<_>>(),
        vec!["abcdefghij ", &format!("{MARKER_1}trailing")],
    );
}
