...
// Degenerate render widths must clamp `content_width` to one cell so
// the box still renders content rows instead of collapsing to zero
// rows. The next three tests pin that behavior across the boundary
// cases.

#[test]
fn degenerate_width_clamps_content_width_to_one_cell() {
    // padding_x=1 on a width-2 box: content_width clamps to 1 and the
    // box renders at one content cell, not zero rows.
    let mut tb = TextBox::new(1, 1);
...
fn zero_render_width_clamps_content_width_to_one_cell() {
    // width = 0 with padding_x = 0: content_width clamps to 1, and the
    // pad-and-bg pipeline still runs at width 0. Children get a
    // one-cell render width.
...
fn padding_x_exceeding_half_width_clamps_to_one_cell() {
    ...
    // saturates to 0, then clamps to 1, so the box still renders a
    // content row instead of collapsing to zero rows.
    ...
    // width; the `CountingLines` fixture does not. The point here is
    // that the box doesn't collapse to zero rows.)
...
fn empty_children_does_not_invalidate_cache() {
    // `add_child` invalidates the cache, but the children-empty early
    // return path does not. Since `clear()` and `remove_child_by_ref`
    // already invalidate, the only way to reach the early return with
    // a stale cache is via this child-empty path. Rendering with no
    // children returns `[]` either way; this test walks the sequence
    // to pin the contract.
    let mut tb = TextBox::new(1, 1);
...
// Padding (top, bottom) and content rows must all route through one
// pad-and-bg pipeline so that:
//   - the no-bg path right-pads content rows to the full render width
//     (otherwise the right side of each row would show whatever the
//     terminal previously displayed);
//   - the bg path applies the closure identically to padding rows and
//     content rows.
// The next three tests pin both halves of that contract.

#[test]
fn no_bg_content_row_right_pads_to_full_width() {
    // A row whose child returned text shorter than the content width
    // must still render at the full byte width.
    let mut tb = TextBox::new(1, 1);
...
#[test]
fn no_bg_padding_row_is_exactly_width_spaces() {
...
// `remove_child_by_ref` matches by data-pointer identity and is a
// no-op (no cache invalidation, returns `None`) when the child isn't
// present. The next three tests pin the happy path, the miss path,
// and that field-equality is not enough to match.
...
fn remove_child_by_ref_does_not_match_a_distinct_instance_with_identical_fields() {
    // Identity (data-pointer) match, not field equality: a
    // separately-constructed `CountingLines` with the exact same
    // `lines` vec must not match an existing child.
    let mut tb = TextBox::new(1, 1);
