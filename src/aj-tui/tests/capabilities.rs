...
#[test]
#[serial_test::serial]
fn cmux_workspace_id_disables_hyperlinks_and_images() {
    // cmux (a tmux-based workspace multiplexer used in some agent
    // harnesses) corrupts image protocols and OSC 8 the same way
    // tmux does, so the host's `CMUX_WORKSPACE_ID` env var gates the
    // conservative fallback even on terminals that would otherwise
    // advertise the features.
    let _guard = isolated_env();
    let _guard2 = with_env(&[
        ("CMUX_WORKSPACE_ID", Some("ws-12345")),
        ("COLORTERM", Some("truecolor")),
    ...
    assert!(
        caps.true_color,
        "colorterm still drives true_color under cmux"
    );
}
...
// at the link-render site. Every test below mutates the cap cache via
// `set_capabilities` (or `isolated_env` for the conservative-default
// case) and asserts on the rendered byte stream.
//
// Two parens-fallback cases plus three OSC 8 cases cover the two
// branches of the link renderer. The `does_not_duplicate_*` tests in
