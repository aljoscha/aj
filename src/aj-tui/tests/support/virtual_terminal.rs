...
impl VirtualTerminal {
    ...
    /// also clear scrollback is intentionally omitted). Use
    /// [`Self::reset`] if a test needs to start from a pristine parser
    /// with no scrollback.
...
impl Terminal for VirtualTerminal {
    ...
    /// same code path.
    ///
    /// `flush` is part of the [`Terminal`] trait so the real process
    /// terminal can flush its writer; the virtual terminal's parser is
    /// synchronous, so the implementation here is a no-op.
    fn flush(&mut self) {
