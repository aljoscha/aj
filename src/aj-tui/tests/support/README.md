...
- `tests/support.rs` — top-level module declarations (re-exports and the
  sync `wait_for_render`). Uses `#[path]` to load sibling files from
...
- `virtual_terminal.rs` — `VirtualTerminal`, a [`Terminal`] implementation
  ...
  `italic`, `underline`, `fg`, `bg`, and wide-char flags — use that, or
  `cells_in_row(row)` for a left-to-right snapshot of a whole row, instead
...
- `fixtures.rs` — reusable component fixtures. Right now:
  ...
  - `MutableLines::new()` / `MutableLines::with_lines([...])` — renders
    ...
    clone into `tui.root` and keep the original to call `set(...)` /
...
| Request a render and wait for it                    | `support::wait_for_render(&mut tui)` / `support::request_and_wait_for_render(&mut tui)` |
| Cell-level style access                             | `vt.cell(row, col)` returns an owned `CellInfo`, or `vt.cells_in_row(row)` for a full row snapshot |
...
- **Per-cell style snapshots.** `vt.cell(row, col)` returns an owned
  ...
  `underline`, `inverse`, `is_wide`, and `is_wide_continuation`.
  `vt.cells_in_row(row)` snapshots a whole row left-to-right. A
  byte-stream harness has to reach into the parser for these; here
  they're owned values that drop the parser borrow before the
  assertion runs.
...
- **`support::send_keys` / `request_and_wait_for_render`.** Batch
  ...
  `request_and_wait_for_render(&mut tui)` packs the
...
- **Async-driver tier.** `support::async_tui::{channel_tui, advance,
  ...
  so timing is deterministic. The sync tier (`support::wait_for_render`)
...
3. Use `support::VirtualTerminal`, `support::wait_for_render`,
...
- **`Tui::render()` is unconditional.** There's no throttled-render
  coalescing in the sync engine, so `wait_for_render(&mut tui)` is just
  ...
  `support::request_and_wait_for_render(&mut tui)` wraps both calls
...
`support::wait_for_render(&mut tui)` just invokes `tui.render()`. For the
