...
///
/// Mirrors pi-tui's `terminal.waitForRender()` (`packages/tui/test/virtual-terminal.ts:213`):
/// pi awaits a `process.nextTick` + 20ms `setTimeout` so its throttled
/// render pipeline settles, then flushes pending writes; our async
/// counterpart drives the same shape via tokio's paused clock by
/// walking `next_event` until a `Render` event arrives. Same name,
/// same conceptual contract — "wait for the throttled render to
/// settle."
///
/// Distinct from the sync-tier [`super::render_now`] helper, which is
/// a one-shot `tui.render()` with no waiting. The sync helper is a
/// Rust-side addition that pi has no equivalent for; the engine choice
/// is obvious at the call site through the deliberate name asymmetry
/// (`render_now()` vs `wait_for_render().await`) rather than through
/// renaming the pi-aligned async helper.
///
/// Returns early without rendering if the event loop ends (e.g. after
/// shutdown); in that case the test's own timeout or assertion will
/// surface the problem.
