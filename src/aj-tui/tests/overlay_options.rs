...
use support::{StaticLines, StaticOverlay, VirtualTerminal, wait_for_render};
...
fn overlay_uses_percentage_of_terminal_width() {
    ...
    tui.root.add_child(Box::new(EmptyContent));
    tui.show_overlay(
        Box::new(overlay),
        OverlayOptions {
...
fn overlay_composites_onto_styled_base_content_without_crashing() {
    ...
    tui.root
        .add_child(Box::new(StaticLines::new(vec![styled_line.clone(); 3])));
...
fn overlay_on_base_content_with_osc_hyperlinks_does_not_crash() {
    ...
    tui.root.add_child(Box::new(HyperlinkContent));
