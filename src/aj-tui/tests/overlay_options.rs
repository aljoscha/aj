...
// ---------------------------------------------------------------------------
// Cursor management on overlay show / hide
// ---------------------------------------------------------------------------

#[test]
fn showing_an_overlay_hides_hardware_cursor_synchronously() {
    // Every push hides the cursor in the same call as `show_overlay`,
    // before any further render runs. The next render's cursor-
    // positioning phase will re-show it if a focused component still
    // emits a cursor marker; what's pinned here is the timing.
    ...
    let mut tui = Tui::new(Box::new(terminal.clone()));
    tui.add_child(Box::new(MarkerEmitter));
    render_now(&mut tui);

    // Pre-condition: the child's marker placed the cursor on screen
    // and the engine emitted `\x1b[?25h`.
    assert!(tui.hardware_cursor_currently_shown());
    assert!(terminal.is_cursor_visible());

    // Capture only what `show_overlay` itself writes.
    ...
    assert!(
        ...
        "show_overlay should clear the cursor-shown flag synchronously",
    );
    assert!(
        terminal.writes_joined().contains("\x1b[?25l"),
        "show_overlay should emit the cursor-hide escape before the next render; got writes {:?}",
        &*terminal.writes(),
    );
    assert!(!terminal.is_cursor_visible());
}
...
#[test]
fn draining_overlay_stack_hides_hardware_cursor_synchronously() {
    // Counterpart to the show-side hide: when the removal call empties
    // the overlay stack and the cursor is currently shown, the hide
    // fires in the same call — without waiting for the next render-
    // driven cursor-positioning pass. To exercise the drain path we
    // re-arm the cursor between push and drain by rendering with the
    // overlay placed clear of the marker, so Phase 6 re-shows the
    // cursor and the drain has something to hide.
    use aj_tui::component::{CURSOR_MARKER, Component};
    use aj_tui::impl_component_any;
    ...
    // Path 1: hide_overlay(handle) drains the stack.
    ...
    // The push hide flips state to false; the next render re-shows the
    // cursor because the marker on row 0 is not covered by a row-2
    // overlay.
    render_now(&mut tui);
    assert!(
        tui.hardware_cursor_currently_shown(),
        "render with overlay clear of marker should re-show cursor",
    );
    ...
    assert!(
        !tui.hardware_cursor_currently_shown(),
        "drain via hide_overlay should clear the cursor-shown flag synchronously",
    );
    assert!(
        terminal.writes_joined().contains("\x1b[?25l"),
        "drain via hide_overlay should emit cursor-hide before the next render; got writes {:?}",
        &*terminal.writes(),
    );
    assert!(!terminal.is_cursor_visible());

    // Path 2: hide_topmost_overlay() drains the stack via the no-arg
    // entry point. Re-arm with a fresh push + render.
    tui.show_overlay(
        Box::new(EmptyContent),
        OverlayOptions {
            anchor: OverlayAnchor::BottomLeft,
    ...
    assert!(
        !tui.hardware_cursor_currently_shown(),
        "drain via hide_topmost_overlay should clear the cursor-shown flag synchronously",
    );
    assert!(
        terminal.writes_joined().contains("\x1b[?25l"),
        "drain via hide_topmost_overlay should emit cursor-hide; got writes {:?}",
        &*terminal.writes(),
    );
    assert!(!terminal.is_cursor_visible());
}
...
#[test]
fn popping_a_non_draining_overlay_does_not_emit_cursor_hide() {
    // Removing an overlay that leaves the stack non-empty must not
    // emit a synchronous cursor-hide on its own. The next render
    // decides cursor visibility based on the still-present overlays.
    use aj_tui::component::{CURSOR_MARKER, Component};
    use aj_tui::impl_component_any;
    ...
    // Two overlays placed clear of the marker so a render in between
    // can re-show the cursor.
    tui.show_overlay(
        Box::new(EmptyContent),
        OverlayOptions {
            anchor: OverlayAnchor::BottomLeft,
    ...
    render_now(&mut tui);
    let cursor_shown_before = tui.hardware_cursor_currently_shown();

    // Capture only what the non-draining pop itself writes.
    terminal.clear_writes();
    assert!(tui.hide_topmost_overlay());

    assert!(
        !terminal.writes_joined().contains("\x1b[?25l"),
        "non-draining pop must not emit cursor-hide on its own; got writes {:?}",
        &*terminal.writes(),
    );
    assert_eq!(
        tui.hardware_cursor_currently_shown(),
        cursor_shown_before,
        "non-draining pop must leave the cursor-shown flag untouched",
    );
}
