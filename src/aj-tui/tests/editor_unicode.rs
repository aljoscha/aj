...
fn renders_isolated_thai_and_lao_am_clusters_without_width_drift() {
    // Thai SARA AM (`U+0E33`) and Lao SIGN AM (`U+0EB3`) sit in their
    // host script as spacing letters but break some terminals' width
    // accounting when isolated. The grapheme-width math in `ansi.rs`
    // keeps the cell count stable; this test pins that the editor's
    // render output has uniform `visible_width` rows for these
    // characters at a narrow width.
    for text in ["ำabc", "ຳabc"] {
        let mut e = editor();
        let width = 8;
        e.set_text(text);
        for line in e.render(width) {
            assert_eq!(
