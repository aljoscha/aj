...
fn on_abort_callback_fires_on_every_cancel_press() {
    // `handle_input` calls `cancel.cancel(); on_abort?.()`
    // unconditionally on every matching key press. Cancellation
    // itself is idempotent (the token only flips false→true once);
    // the callback is not.
    ...
    // Subsequent cancel-key presses still fire the callback; the
    // underlying token stays cancelled either way.
    loader.handle_input(&Key::escape());
    loader.handle_input(&Key::escape());
...
#[test]
fn invalidate_forwards_to_the_wrapped_loader() {
    // `CancellableLoader` wraps a `Loader` whose embedded `Text` body
    // caches its output on `(text, width)`. The framework's
    // `tui.invalidate()` walks every owned component to clear those
    // caches on a global render-affecting event (resize, theme
    // palette swap). Without forwarding, the wrapped loader's body
    // cache would survive the invalidation. Black-box check: render
    // → invalidate → render produces the same shape and doesn't
    // panic.
    let mut loader = CancellableLoader::with_identity_styles(RenderHandle::detached(), "working");
