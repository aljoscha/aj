...
fn arrow_keys_wrap_around_and_emit_selection_changes() {
    ...
    // Record every selection-change callback so we can assert it fired
    // and carried the new item.
    ...
    assert_eq!(
        *changes.borrow(),
        ...
        "on_selection_change should fire once per arrow-key navigation, with the newly selected item",
    );
}

/// Readline-style `Ctrl+P` / `Ctrl+N` are bundled with the arrow keys
/// as defaults for `tui.select.up` / `tui.select.down`, so a `SelectList`
/// can be navigated without leaving the home row. Mirrors the arrow-key
/// wrap-around test above to lock in identical behavior for the aliases.
#[test]
fn ctrl_p_and_ctrl_n_navigate_select_list_like_arrow_keys() {
    use aj_tui::keys::Key;
    ...
    // Ctrl+P from index 0 wraps to the last item, just like Up.
    let consumed = list.handle_input(&Key::ctrl('p'));
    assert!(consumed, "ctrl+p must be consumed by SelectList");
    assert_eq!(list.selected_item().map(|i| i.value.as_str()), Some("c"));

    // Ctrl+N wraps back to the first item, just like Down.
    let consumed = list.handle_input(&Key::ctrl('n'));
    assert!(consumed, "ctrl+n must be consumed by SelectList");
    assert_eq!(list.selected_item().map(|i| i.value.as_str()), Some("a"));
}

// ---------------------------------------------------------------------------
// Stateless centering scroll model
//
// These tests lock in the chosen scroll model: the visible window is
// recomputed from `selected` on every render via
// clamp(selected - max_visible / 2, 0, len - max_visible). They cover
// sequential navigation (selection floats near center), wraparound at
// both ends of a long list (window clamps to top/bottom edge), and
// filter resets.
// ---------------------------------------------------------------------------
...
fn sequential_down_navigation_centers_selection_in_window() {
    ...
    // Press Down 4 times — selected becomes 4. With the centering
...
fn scroll_info_is_truncated_to_terminal_width_minus_two() {
    // The `(N/TOTAL)` indicator is truncated to `width - 2` so it
    // doesn't bleed past the right edge of narrow overlays.
