...
use aj_tui::ansi::{normalize_terminal_output, truncate_to_width, visible_width};
...
fn visible_width_keeps_thai_and_lao_am_clusters_at_their_normal_cell_width() {
    // Standalone SARA AM vowels — one cell each.
    assert_eq!(visible_width("ำ"), 1);
    assert_eq!(visible_width("ຳ"), 1);
    // Consonant + AM vowel cluster as one grapheme but still occupy two cells.
