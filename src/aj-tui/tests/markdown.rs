...
#[test]
fn rejects_strikethrough_when_open_is_followed_by_whitespace() {
    // Strict double-tilde delimiters require non-whitespace boundaries:
    // loose tildes wrapping a phrase like `~~ foo ~~` should render
    // literally rather than struck. Locks in the "no strikethrough on
    ...
    assert!(
        plain.contains("~~ foo ~~"),
        "loose tildes around whitespace should remain visible; got: {:?}",
        plain,
    );
    assert!(
        !joined.contains("\x1b[9m"),
        "loose-tilde text should not use strikethrough styling; got: {:?}",
        joined,
    );
}

#[test]
fn rejects_strikethrough_when_close_is_preceded_by_whitespace() {
    // Strict double-tilde delimiters: trailing whitespace inside the
    // span (`~~foo ~~`) is also a non-strikethrough loose-tilde shape.
    // Locks in the "no strikethrough on whitespace-padded closings"
    ...
    assert!(
        plain.contains("~~foo ~~"),
        "loose tildes with trailing whitespace should remain visible; got: {:?}",
        plain,
    );
    assert!(
        !joined.contains("\x1b[9m"),
        "trailing-whitespace tilde span should not strikethrough; got: {:?}",
        joined,
    );
}

#[test]
fn rejects_strikethrough_when_close_run_exceeds_two_tildes() {
    // A run of more than two closing tildes (`~~foo~~~`) is rejected
    // by the strict rule — the third `~` would otherwise leave a
    // stray tilde after a struck `foo`. Decorative runs like
    ...
    for source in ["before ~~foo~~~ after", "a ~~~~~~~~ b"] {
        ...
        assert!(
            !joined.contains("\x1b[9m"),
            "long tilde run in {:?} must not activate strikethrough; got: {:?}",
            source,
            joined,
        );
        assert!(
            plain.contains(source),
            "long tilde run in {:?} must remain visible verbatim; got: {:?}",
            source,
            plain,
        );
    }
