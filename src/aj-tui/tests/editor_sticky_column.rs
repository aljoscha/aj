...
//! clamps the cursor's column to the shorter line's end — but
//! remembers that visual column so the next vertical move back up
//! (or further down through more short lines) lands back at that
//! column if it fits. This matches the Emacs / readline /
//! modern-editor behavior that users expect.
//!
...
/// editor exposes no `set_cursor` API: cursor position is always
/// the result of input.
