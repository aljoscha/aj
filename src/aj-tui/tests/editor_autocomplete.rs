...
// ---------------------------------------------------------------------------
//
// Slash-command popups get a tighter `[12, 32]` primary-column bound
// while every other prefix (`@`-style file completion, etc.) takes
// the layout default. The two tests below pin the slash branch
// (column 14 = prefix(2) + min(12)) and the default branch (column
// 34 = prefix(2) + default(32)) so the gating logic can't silently
// regress to a single branch.
