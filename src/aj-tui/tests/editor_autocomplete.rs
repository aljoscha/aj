...
// ---------------------------------------------------------------------------
// F50: prefix-discriminated autocomplete popup layout.
// ---------------------------------------------------------------------------
//
// Pi-tui (`editor.ts:210-213, 2074-2080`) gives slash-command popups a
// tighter `[12, 32]` primary-column bound while every other prefix
// (`@`-style file completion, etc.) takes the layout default. The two
// tests below pin the slash branch (column 14 = prefix(2) + min(12)) and
// the default branch (column 34 = prefix(2) + default(32)) so the gating
// logic can't silently regress to a single branch.

#[tokio::test]
async fn slash_prefix_autocomplete_uses_slash_command_layout() {
    use aj_tui::autocomplete::{CombinedAutocompleteProvider, SlashCommand};

    let mut e = editor();
    // Identity theme keeps strip_ansi() output equal to the rendered
    // bytes, so visible_index_of can locate the description text directly.
    e.set_theme(support::themes::identity_editor_theme());
    let provider = CombinedAutocompleteProvider::new(
        vec![
            SlashCommand::new("clear")
    ...
    assert!(
        e.is_showing_autocomplete(),
        "typing `/` should open the slash-command popup",
    );

    let plain = render_plain(&mut e, 60);
    let row_with_clear = plain
        .iter()
        .find(|l| l.contains("clear") && l.contains("Clear all messages"))
        .unwrap_or_else(|| panic!("expected a popup row containing `clear`; got {:?}", plain));

    // With the slash-command layout (`min=12, max=32`) and items whose
    // longest visible width is under 12 cells, the primary column
    // collapses to its minimum. `description_start = prefix_marker(2) +
    // primary_column(12) = 14`. Without the slash branch, the default
    // layout's fixed 32-cell primary column would push descriptions to
    // column 34 instead.
    assert_eq!(
        support::visible_index_of(row_with_clear, "Clear all messages"),
        14,
        "slash-command popup should use the slash layout (`[12, 32]`); \
         description column should be 14, not the default 34. row: {:?}",
        row_with_clear,
    );
}
