...
//! `max(5, floor(rows * 0.3))`. The value is read from the
//! `RenderHandle` published by the owning `Tui`, so tests verify
...
fn max_visible_lines_falls_back_to_five_with_a_detached_handle() {
    ...
    // rather than panicking or returning 0. The detached handle is
    // the canonical "no live terminal" representation.
    let e = Editor::new(
