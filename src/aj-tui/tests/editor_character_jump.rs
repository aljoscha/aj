...
// ---------------------------------------------------------------------------
// Jump-mode falls through control characters silently (H1)
// ---------------------------------------------------------------------------

/// H1 regression. Jump-mode has no explicit `tui.select.cancel`
/// match — only the same-hotkey cancel, the printable-jump arm,
/// and a silent fallthrough that clears `jump_mode` for any
/// control character. An earlier version of the editor added an
/// explicit `KeyCode::Esc` cancel that consumed the key (returned
/// `true`); that swallowed Escape the parent surface would
/// otherwise handle. Esc now silently clears `jump_mode` and falls
/// through to normal handling, returning `false` so the parent
/// sees the key.
#[test]
fn jump_mode_does_not_consume_escape() {
    ...
    assert!(
        !consumed,
        "Escape in jump-mode must not be consumed; it silently \
         clears jump_mode and falls through so the parent can \
         handle Esc",
    );
...
/// H1 regression complement. Jump-mode does not consult
...
/// clears the mode and bubbles up. Every other modal in this
/// crate routes cancel through the registry, but jump-mode is the
/// deliberate exception.
#[test]
#[serial(global_keybindings)]
fn jump_mode_does_not_consume_user_bound_cancel() {
    keybindings::reset();
    ...
    assert!(
        ...
        "Ctrl+G with cancel rebound must not be explicitly consumed \
         by jump-mode; jump-mode does not consult \
         tui.select.cancel",
    );
