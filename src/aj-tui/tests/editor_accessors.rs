...
#[test]
fn set_text_normalizes_carriage_returns_and_expands_tabs_to_spaces() {
    // `\r\n` and lone `\r` collapse to `\n`, and tabs expand to four
    // spaces. The editor's internal model never holds embedded `\r`
    // or `\t`, so any caller (programmatic update, paste, file load)
    // sees the same line shape regardless of input quirks.
    let mut e = editor();
    e.set_text("a\r\nb\rc\td");
    assert_eq!(e.get_text(), "a\nb\nc    d");
}
...
#[test]
fn insert_text_at_cursor_normalizes_carriage_returns_and_expands_tabs_to_spaces() {
    // Same normalization rules as `set_text`; the helper is shared.
    let mut e = editor();
