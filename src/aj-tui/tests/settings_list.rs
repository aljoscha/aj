...
fn typing_a_printable_recomputes_the_filter_after_each_keystroke() {
    ...
    // [`SettingsList::apply_filter`] runs unconditionally — mirroring
    // pi-tui's `settings-list.ts:194-195` shape. This test walks the
    // filter through a narrow / broaden / clear sequence and asserts
    // the filtered list stays in lockstep with the search input's
    // current value at every step.
    let (mut list, _, _) = make_settings_list(
        sample_items(),
        SettingsListOptions {
            enable_search: true,
