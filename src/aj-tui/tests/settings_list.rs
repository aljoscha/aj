...
#[test]
fn typing_a_printable_recomputes_the_filter_after_each_keystroke() {
    // Pin down the "always-recompute" invariant.
    // After every event forwarded to the embedded search input,
    // [`SettingsList::apply_filter`] runs unconditionally. This test
    // walks the filter through a narrow / broaden / clear sequence
    // and asserts the filtered list stays in lockstep with the
    // search input's current value at every step.
    let (mut list, _, _) = make_settings_list(
        sample_items(),
        SettingsListOptions {
            enable_search: true,
    ...
    // Initial: no filter, "theme" is the first (and selected) item.
    assert_eq!(list.selected_id(), Some("theme"));

    // Type 'c' → matches "Confirm Exit" (label contains 'c').
    // The filter must recompute to put "confirm" at the top.
    list.handle_input(&Key::char('c'));
    assert_eq!(list.selected_id(), Some("confirm"));

    // Type 'o' → "Confirm Exit" still matches "co" (subsequence). The
    // filter must recompute and keep "confirm" surfaced.
    list.handle_input(&Key::char('o'));
    assert_eq!(list.selected_id(), Some("confirm"));

    // Type 'z' → no item matches "coz". The filter must recompute and
    // produce an empty filtered list.
    list.handle_input(&Key::char('z'));
    assert_eq!(list.selected_id(), None);

    // Backspace → broadens back to "co", filter recomputes, "confirm"
    // returns.
    list.handle_input(&Key::backspace());
    assert_eq!(list.selected_id(), Some("confirm"));

    // Backspace twice more → search input emptied, filter recomputes
    // to the unfiltered list, "theme" (the original first item) is
    // selected again.
    list.handle_input(&Key::backspace());
    list.handle_input(&Key::backspace());
    assert_eq!(list.selected_id(), Some("theme"));
...
#[test]
fn render_does_not_panic_at_widths_too_narrow_to_fit_a_row() {
    // At very narrow terminal widths the per-row prefix + label +
    // separator + value composition would otherwise produce arithmetic
    // that underflows (value max width = `width - used - 2`). The
    // value-column allocation goes through `saturating_sub` so the
    // value slot is at worst 0, and every emitted item / hint /
    // no-match line is fed through `truncate_to_width(_, width, …)`
    // so the differential renderer never sees a row wider than
    // `width`. Render must stay panic-free across the empty,
    // populated, no-match, and submenu-active branches.
    let widths = [1usize, 2, 3, 5, 10, 15];

    // Items + cursor row (selects "theme" by default).
    let (mut populated, _, _) = make_settings_list(
        sample_items(),
        SettingsListOptions {
            enable_search: true,
        },
    ...
    for width in widths {
        let _ = populated.render(width);
    }

    // Empty list, search enabled (exercises both `items` and
    // `search` empty-state branches).
    let (mut empty, _, _) = make_settings_list(
        Vec::new(),
        SettingsListOptions {
            enable_search: true,
        },
    ...
    for width in widths {
        let _ = empty.render(width);
    }

    // No-match branch with description-bearing item selected.
    let (mut no_match, _, _) = make_settings_list(
        sample_items(),
        SettingsListOptions {
            enable_search: true,
        },
    );
    no_match.handle_input(&Key::char('z'));
    for width in widths {
        let _ = no_match.render(width);
    }
}
...
fn make_list_with_submenu() -> (
    ...
    let factory = Box::new(move |current: &str, done: SubmenuDoneCallback| {
        ...
        *last_submenu_log_clone.borrow_mut() = Some(log);
        let boxed: Box<dyn Component> = Box::new(submenu);
        boxed
    });
...
#[test]
fn invalidate_forwards_to_the_active_submenu() {
    // When the framework signals a global render-affecting change
    // (terminal resize, theme palette swap), the parent's
    // `invalidate()` walks every owned component. SettingsList holds
    // its submenu inside an internal slot — not as a `Container`
    // child — so without explicit forwarding the submenu's own
    // caches would survive the invalidation.
    //
    // The fixture: a settings list whose factory returns a stub
    // component that increments a shared counter on every
    // `invalidate()` call. Open the submenu, then call invalidate on
    // the parent and assert the counter ticked.

    /// Inline stub with an invalidate counter. Kept local so the
    /// outer-file `StubSubmenu` doesn't grow a field that's only
    /// useful here.
    struct CountingSubmenu {
        invalidate_count: Rc<RefCell<u32>>,
    }
    ...
    let invalidate_count = Rc::new(RefCell::new(0u32));
    let count_for_factory = Rc::clone(&invalidate_count);

    let factory = Box::new(move |_current: &str, _done: SubmenuDoneCallback| {
        let component = CountingSubmenu {
            invalidate_count: Rc::clone(&count_for_factory),
        };
        let boxed: Box<dyn Component> = Box::new(component);
        boxed
    });

    let items = vec![SettingItem::with_submenu("picker", "Picker", "x", factory)];
    ...
    // No submenu open: invalidate is a no-op (no submenu to forward
    // to) and must not panic.
    list.invalidate();
    assert_eq!(*invalidate_count.borrow(), 0);

    // Open the submenu and confirm forwarding fires once per
    // invalidate() call on the parent.
    list.handle_input(&Key::enter());
    assert!(list.has_active_submenu());

    list.invalidate();
    assert_eq!(*invalidate_count.borrow(), 1);
