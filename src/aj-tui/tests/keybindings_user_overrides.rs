//! End-to-end tests that a user override installed via
//! [`aj_tui::keybindings::set_user_bindings`] actually takes effect on
//! the components that consume the registry — Editor, TextInput,
//! SelectList, SettingsList, CancellableLoader.
//!
//! Each component routes its key handling through the process-wide
//! [`KeybindingsManager`]; these tests exercise that wiring with a
//! representative override and confirm:
//!
...
//!
//! The tests run serially under the `global_keybindings` group
...
// ---------------------------------------------------------------------------
// Per-component rebind-and-fire coverage.
//
// Each test below picks a representative override for one consumer,
// drives the rebound key through the component to confirm the action
// fires, then drives the previous default to confirm it no longer
// does.
// ---------------------------------------------------------------------------

/// `TextInput` consumes `tui.editor.deleteCharBackward` (default `backspace`).
...
fn rebinding_tui_editor_delete_char_backward_takes_effect_on_input() {
    ...
    assert!(
        consumed,
        "ctrl+h must be consumed by TextInput after the rebind",
    );
    assert_eq!(
        input.value(),
        "hell",
    ...
    // so the TextInput doesn't consume it and the value is unchanged.
...
fn rebinding_tui_select_confirm_takes_effect_on_settings_list() {
    ...
    // this rebind. That invariant is exercised separately by
    // `tests/settings_list.rs`; this test stays narrowly focused on
    // the registry-driven rebind shape.
