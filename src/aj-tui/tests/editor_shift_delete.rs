...
//! `shift+delete` because they're matched inline (outside the
//! registry) via `key_id_matches(event, "shift+backspace")` alongside
//! the registry check, so users on terminals that report
