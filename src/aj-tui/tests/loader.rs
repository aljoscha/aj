...
fn default_indicator_renders_a_braille_spinner_frame() {
    ...
    // width` (Loader's embedded Text uses padding_x=1, padding_y=0
    // — emitting left/right margins and padding to the full render
    // width).
...
// configured with `padding_x = 1, padding_y = 0`, then prepends a
// leading blank row. So:
...
fn loader_indents_spinner_one_space_to_match_text_padding_x_1() {
    // Loader's embedded Text has `padding_x = 1`, so the Text
    // component puts a single leading space before the spinner
    // glyph. A regression that reverted to
...
fn loader_pads_each_row_to_the_full_render_width() {
    ...
    // (`leftMargin + line + rightMargin + spaces`). A shape that
    // emitted `frame + " " + msg` with no trailing pad would leave
    // the right side of the row at whatever the terminal had
    // previously.
    let mut loader = Loader::with_identity_styles(RenderHandle::detached(), "hi");
    loader.set_indicator(Some(LoaderIndicatorOptions::with_frames(["*"])));
...
fn long_message_wraps_to_render_width_minus_padding_x_pair() {
    // Text wraps to `max(1, width - 2 * padding_x)`, so a message
    // that doesn't fit on one row spills to a second row through
    // the embedded Text.
...
#[test]
fn set_indicator_after_stop_reactivates_the_loader() {
    // Public contract: `set_indicator` says "show this indicator now",
    // so it should resume animation if the loader was previously
    // stopped. Without this, a stop → set_indicator pair would silently
    // leave the loader inactive and the new indicator would never
    // animate.
    let mut loader = Loader::with_identity_styles(RenderHandle::detached(), "x");
    loader.stop();
    assert!(
        !loader.is_active(),
        "stop() should leave the loader inactive"
    );

    loader.set_indicator(Some(LoaderIndicatorOptions::with_frames(["A", "B"])));
    assert!(
        loader.is_active(),
        "set_indicator should reactivate a stopped loader",
    );
}
...
#[test]
fn invalidate_does_not_panic_and_render_still_produces_a_valid_frame() {
    // The framework calls `invalidate()` on every component when a
    // global render-affecting event happens (resize, palette swap).
    // The Loader composes its body through an embedded Text whose
    // render output is cached on `(text, width)`. Without the
    // forwarding `invalidate` impl, the body's cache would survive
    // the invalidation until `set_text` was next called with a
    // different value (which only happens when the message or active
    // frame changes).
    //
    // Black-box the cache via behavior: render → invalidate → render
    // and assert both calls return the expected shape. The point is
    // structural (the forwarding exists; it doesn't panic; output
    // stays correct), not perf — there's no externally observable
    // difference between a cache-hit and a recomputed render that's
    // byte-identical to the cached lines.
    let mut loader = Loader::with_identity_styles(RenderHandle::detached(), "still");
    loader.set_indicator(Some(LoaderIndicatorOptions::with_frames(["*"])));
    ...
    assert_eq!(before, after, "invalidation must not change static output");
}
...
async fn loader_animation_pump_does_not_run_for_static_single_frame_indicator() {
    ...
    // construction + set_indicator each request a synchronous
...
async fn loader_animation_pump_stops_when_loader_stop_is_called() {
    ...
    // (construction itself also requests a render). The longer 32ms
    // timeout is needed because `support::async_tui::drain_ready`
...
// ---------------------------------------------------------------------------
// Synchronous render-request points
//
...
async fn set_message_on_a_static_loader_requests_a_render() {
    ...
    // synchronously), then mutate the message and assert that the
...
async fn set_indicator_swap_requests_a_render_even_when_new_indicator_is_static() {
    ...
    // to a different static indicator. Without the synchronous
    // request, the swap would sit unrendered (no animation pump
    // runs for a static frame, no input is arriving, and
    // `set_initial_render(false)` keeps the implicit bootstrap
    // render off). With it, set_indicator's tail calls
    // request_repaint and the swap surfaces immediately.
    use aj_tui::components::loader::{Loader, LoaderIndicatorOptions};
    use aj_tui::tui::{Tui, TuiEvent};
    use std::time::Duration;
...
async fn freshly_constructed_loader_requests_a_render_with_initial_render_disabled() {
    ...
    //
    // Without that synchronous request, an idle Tui with
    // `initial_render = false` and a freshly-constructed multi-frame
    // loader appears one `interval` late (first pump tick), and a
    // single-frame / empty-frame loader doesn't appear at all until
    // a separate render trigger fires.
    use aj_tui::components::loader::{Loader, LoaderIndicatorOptions};
    use aj_tui::tui::{Tui, TuiEvent};
    use std::time::Duration;
