//! Regression tests for the editor cursor cell terminator.
//!
//! The cursor cell terminates with a full SGR reset (`\x1b[0m`), not
//! reverse-video-off only (`\x1b[27m`). Without the full reset,
//! foreground/background/attribute styling that is open in the line
//! text *before* the cursor cell continues to apply to cells *after*
...
fn styling_open_before_cursor_does_not_bleed_past_the_cursor_cell() {
    ...
    assert_eq!(
        ...
        "cursor cell must terminate with `\\x1b[0m` so a red `\\x1b[31m` open \
         in the line text does not bleed past the cursor. \
         Rendered: {joined:?}",
    );
}
