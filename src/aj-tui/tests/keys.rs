...
fn tab_variants_match_their_descriptors() {
    ...
    // Modern Kitty terminals report `Shift+Tab` as `Tab + SHIFT`.
    assert!(key_id_matches(
        &key(KeyCode::Tab, KeyModifiers::SHIFT),
        "shift+tab",
    ));
    // Legacy `\x1b[Z` arrives as `BackTab + SHIFT` from crossterm; the
    // matcher folds it to `Tab + SHIFT` so the same descriptor works
    // across both shapes.
    assert!(key_id_matches(
        &key(KeyCode::BackTab, KeyModifiers::SHIFT),
        "shift+tab",
    ));
    // BackTab without SHIFT shouldn't slip through as plain `tab`.
    assert!(!key_id_matches(
        &key(KeyCode::BackTab, KeyModifiers::SHIFT),
        "tab",
    ));
...
fn modifier_order_in_descriptor_does_not_matter() {
    // The matcher is order-independent (it checks modifier bitflags),
    // so descriptor token order should not affect the result.
    let event = key(
        KeyCode::Char('a'),
        KeyModifiers::CONTROL | KeyModifiers::SHIFT,
    );
...
fn common_named_key_aliases_match() {
    // `return` and `enter` are both accepted; `esc` and `escape` are both
    // accepted. Keep these pinned in case the alias list shrinks again.
    assert!(key_id_matches(
        &key(KeyCode::Enter, KeyModifiers::NONE),
        "return"
    ));
...
fn function_key_aliases_are_case_insensitive() {
    // F keys accept both "f5" and "F5" — the matcher case-folds the
    // descriptor before parsing.
...
fn named_key_descriptors_are_case_insensitive() {
    // Same case-folding invariant as function keys, for named keys:
    // "enter" vs "Enter" vs "ENTER" all match.
...
fn paste_event_does_not_match_char_descriptor() {
    ...
    // Matching a paste as a keystroke would confuse keybinding dispatch.
