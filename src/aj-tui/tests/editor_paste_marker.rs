...
//! doesn't have to page through hundreds of characters to move past
//! a paste.
//!
//! The tests come in two flavors:
//!
//! - **Functional**: navigation, deletion, expansion, submit, tab
//!   handling, path-prefix safety.
//! - **Layout-interaction**: cursor snapping through marker visual
//!   lines on narrow terminals, wrapping at marker boundaries,
//!   sticky-column behavior across marker visual lines. These rely
//!   on `render(width)` being called between cursor moves so the
...
fn preserves_sticky_column_when_navigating_through_paste_marker_line() {
    ...
    // binding. (Plain Enter would match `tui.input.submit`, which is
    // a silent no-op under `disable_submit = true`.)
...
fn does_not_get_stuck_moving_down_from_a_multi_visual_line_paste_marker() {
    ...
    // Enter is a no-op under `disable_submit = true`.
    e.handle_input(&Key::shift_enter());
