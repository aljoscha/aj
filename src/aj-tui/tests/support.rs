...
/// wait for. The sync engine is a Rust-side addition that pi-tui has
/// no equivalent for; pi's only render-driver helper
/// (`terminal.waitForRender()`) is the async-tier counterpart, mirrored
/// by [`async_tui::wait_for_render`] under the same name.
///
/// The deliberate name asymmetry — `render_now()` vs
/// `wait_for_render().await` — makes the engine choice obvious at the
/// call site without having to rename the pi-aligned async helper.
