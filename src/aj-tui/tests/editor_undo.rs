...
/// CSI-u Ctrl+letter sequences inside a bracketed paste. tmux
/// popups configured with `extended-keys-format=csi-u` re-encode
/// embedded `\n` bytes as `\x1b[106;5u` (Ctrl+J) inside the paste
/// payload. The editor must decode those back to `\n` *before* its
/// per-char control-byte filter runs, otherwise the filter strips
/// the `\x1b` and leaks the printable tail (`[106;5u`) between the
/// lines.
#[test]
fn decodes_csi_u_ctrl_letters_inside_bracketed_paste() {
    let mut e = editor();
    e.handle_input(&aj_tui::keys::InputEvent::Paste(
        "line1\x1b[106;5uline2\x1b[106;5uline3".to_string(),
    ));
