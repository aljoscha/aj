...
/// `tui.select.up` and `tui.select.down` ship with readline-style
/// `ctrl+p` / `ctrl+n` aliases alongside the arrow keys, so list-style
/// navigation is reachable without leaving the home row. Pinned so the
/// defaults can't silently drift back to arrow-only.
#[test]
fn select_navigation_defaults_include_emacs_aliases() {
    let kbm = KeybindingsManager::new(tui_keybindings(), Vec::<(&str, &str)>::new());

    assert_eq!(kbm.get_keys("tui.select.up"), &["up", "ctrl+p"]);
    assert_eq!(kbm.get_keys("tui.select.down"), &["down", "ctrl+n"]);

    use aj_tui::keys::Key;
    assert!(kbm.matches(&Key::up(), "tui.select.up"));
    assert!(kbm.matches(&Key::ctrl('p'), "tui.select.up"));
    assert!(kbm.matches(&Key::down(), "tui.select.down"));
    assert!(kbm.matches(&Key::ctrl('n'), "tui.select.down"));
}

/// A user override that resolves to an empty key list means the user
/// has *unbound* the action. The resolved key list is empty, `matches`
/// returns false for what used to be the default, and unrelated
/// actions are untouched.
///
/// `rebuild()` stores the already-normalized empty Vec under the
/// action ID and reads it back verbatim, so an explicit empty override
/// shadows the default. Pin the behavior so it can't regress.
#[test]
fn empty_user_override_removes_the_binding() {
    let kbm = KeybindingsManager::new(
        tui_keybindings(),
        [("tui.input.submit", Vec::<&str>::new())],
    );

    // The action has no resolved keys.
    assert!(kbm.get_keys("tui.input.submit").is_empty());
    // The previous default key no longer matches.
    use aj_tui::keys::Key;
    assert!(!kbm.matches(&Key::enter(), "tui.input.submit"));
    // The action still appears in `get_resolved_bindings` (with empty
    // keys) so callers iterating definitions can still see it.
    let resolved = kbm.get_resolved_bindings();
    let entry = resolved
        .iter()
        .find(|(id, _)| id == "tui.input.submit")
        .expect("tui.input.submit is still listed");
    assert!(entry.1.is_empty());
    // An unrelated action is untouched.
    assert_eq!(kbm.get_keys("tui.select.confirm"), &["enter"]);
    // No conflict is reported for a removed action.
    assert!(kbm.get_conflicts().is_empty());
}
