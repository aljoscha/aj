...
#[test]
fn wrap_text_with_ansi_breaks_long_word_with_embedded_tab() {
    // Regression: `break_long_word` previously counted a tab grapheme
    // as zero columns (control-char short-circuit) instead of three,
    // so a long unbroken word containing a tab was under-counted and
    // the resulting chunk overflowed the requested width. With the fix
    // the tab contributes its expanded width when deciding the break;
    // the trailing tab is then stripped by the per-line `trim_end`
    // pass, leaving two clean 7-column chunks.
    let line = "aaaaaaa\tbbbbbbb"; // 7 + 3 (tab) + 7 = 17 visible columns
    let wrapped = wrap_text_with_ansi(line, 10);

    // Width contract: every chunk must fit in 10 columns.
    for chunk in &wrapped {
        let w = visible_width(chunk);
        assert!(
            w <= 10,
            "chunk {chunk:?} has visible width {w}, expected <= 10",
        );
    }

    // Concrete shape: the break lands at the tab, both halves are 7
    // columns, and the trailing tab is trimmed.
    assert_eq!(wrapped, vec!["aaaaaaa".to_string(), "bbbbbbb".to_string()]);
}
...
#[test]
fn osc_8_preserves_bel_terminators_when_wrapping_oauth_style_hyperlinks() {
    // OAuth login URLs are typically advertised with a BEL terminator
    // because some terminals only treat BEL-terminated hyperlinks as
    // clickable. Wrapping must reopen the link on each continuation
    // line using the *same* terminator and never silently flip BEL→ST.
    let url = format!("https://example.com/oauth/{}", "a".repeat(32));
    let input = format!("\x1b]8;;{}\x07{}\x1b]8;;\x07", url, url);
    let lines = wrap_text_with_ansi(&input, 20);

    assert!(lines.len() > 1, "input should wrap to multiple lines");

    let bel_open = format!("\x1b]8;;{}\x07", url);
    let st_open = format!("\x1b]8;;{}\x1b\\", url);
    ...
    for line in &lines {
        ...
        assert!(
            ...
            "line {:?} reopens the hyperlink with ST instead of BEL",
            line,
        );
    }

    // Every non-final line must close the open hyperlink before the
    // wrap point, also using BEL.
    for line in lines.iter().take(lines.len().saturating_sub(1)) {
        assert!(
...
/// string. Used by `osc_8_open_is_re_emitted_at_the_start_of_continuation_lines`
/// to inspect just the visible (and SGR-styled) content of a wrapped
/// line; kept byte-oriented so it doesn't pull in a regex dependency.
