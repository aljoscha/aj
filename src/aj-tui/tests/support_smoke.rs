...
use support::{StaticLines, VirtualTerminal, request_and_wait_for_render, wait_for_render};
...
fn virtual_terminal_round_trips_a_render() {
    ...
    tui.root
        .add_child(Box::new(StaticLines::new(["hello", "world"])));
...
#[test]
fn request_and_wait_for_render_drives_a_full_render_pass() {
    ...
    let mut tui = Tui::new(Box::new(terminal.clone()));
    tui.root.add_child(Box::new(StaticLines::new(["initial"])));

    request_and_wait_for_render(&mut tui);
