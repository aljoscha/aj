...
//!
//! The editor's newline branch fires not just for the registry-
...
//! **regardless of `disable_submit`** so a terminal whose Shift+Enter
...
// ---------------------------------------------------------------------------
// `disable_submit = true`: byte-form newline fallbacks insert a literal
// newline regardless of the submit-key gate.
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// disable_submit = false: byte fallbacks fire regardless of
// `disable_submit`, so the same byte forms must insert a newline
// here too. Plain Enter still submits (the fallback excludes plain
// `KeyCode::Enter`).
// ---------------------------------------------------------------------------
...
#[test]
#[serial(global_keybindings)]
fn submit_trims_leading_and_trailing_whitespace_from_the_delivered_text() {
    // `take_submitted` (and the `on_submit` callback) deliver the
    // message body trimmed of leading and trailing whitespace.
    // Without trimming, accidental spaces from typing or pasted
    // padding lines would leak through to the consumer.
    keybindings::reset();
    let mut e = Editor::new(
        RenderHandle::detached(),
        support::themes::default_editor_theme(),
    ...
    e.set_text("   hello   ");
    e.handle_input(&Key::enter());
    assert_eq!(e.take_submitted(), Some("hello".to_string()));
}
...
// ---------------------------------------------------------------------------
// `disable_submit` + the submit-key. The submit branch tests
// `disable_submit` *inside* the branch and returns silently — no
// "submit-key becomes newline" fallback. The submit key is consumed
// (the editor returns `true`) without inserting a newline, falling
...
fn rebound_submit_is_silently_consumed_in_disable_submit_mode() {
    ...
    // the rebound submit key is consumed silently.
    keybindings::reset();
    keybindings::set_user_bindings([("tui.input.submit", "ctrl+enter")]);
    let mut e = Editor::new(
        RenderHandle::detached(),
...
fn default_submit_is_silently_consumed_in_disable_submit_mode() {
    ...
    // `disable_submit = true` is a silent no-op. Newlines come from
    // Shift+Enter via `tui.input.newLine`, not from the submit key.
    keybindings::reset();
    let mut e = Editor::new(
        RenderHandle::detached(),
        support::themes::default_editor_theme(),
