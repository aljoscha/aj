...
fn non_capturing_overlay_renders_even_though_it_does_not_capture_input() {
    ...
    tui.add_child(Box::new(StaticLines::new(["base"])));
    tui.show_overlay(
        Box::new(Overlay),
        OverlayOptions {
...
#[test]
fn hide_topmost_overlay_pops_a_non_capturing_top_without_reassigning_focus() {
    // Stack (bottom→top): capturing, then non-capturing. The
    // non-capturing overlay is on top of the stack but does not hold
    // focus (capturing was auto-focused on show). hide_topmost_overlay
    // pops the topmost entry regardless of capturing/non-capturing,
    // and because the popped overlay didn't own focus, the current
    // focus holder (capturing) is left untouched.
    let terminal = VirtualTerminal::new(80, 24);
    let mut tui = Tui::new(Box::new(terminal.clone()));
    ...
    // Topmost is the non-capturing overlay. It pops, but since it
    // didn't hold focus, the capturing overlay remains focused.
...
fn capturing_overlay_hidden_and_shown_again_renders_on_top() {
    ...
    //   5. Unhide `B`. Unhiding bumps `B`'s focus_order so it
