...
impl ToolDefinition for EditFileTool {
    ...
    fn execute(
        ...
        let content = turn_state
            .get_staged_file_content(&path_buf)
            .cloned()
            .unwrap_or_else(|| {
                fs::read_to_string(&input.path)
                    .unwrap_or_else(|e| panic!("Failed to read file '{}': {}", input.path, e))
            });
        ...
        if matches.len() == 1 || input.replace_all {
            ...
            let original_content = if turn_state.get_staged_file_content(&path_buf).is_some() {
                None
            } else {
                Some(content.clone())
            };

            turn_state.stage_file_modification(path_buf.clone(), original_content, new_content);
