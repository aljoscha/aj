...
//! quoted paths.
//!
//! The provider implements the [`AutocompleteProvider`] /
//! [`AutocompleteSession`] traits the [`vaxis::vxfw::TextArea`] editor
//! drives. The traits and the data types they exchange live in the widget
//! crate. This module supplies the concrete, filesystem-backed behavior.
//!
//! # Async & cancellation
//!
//! [`AutocompleteProvider::get_suggestions`] is async and takes a
//! [`tokio_util::sync::CancellationToken`]. The filesystem walks honor the
//! token promptly: they run inside a [`tokio::task::spawn_blocking`] worker,
//! so callers can abort a long-running walk by cancelling the token. The
//! `@`-fuzzy search uses [`ignore::WalkBuilder::build_parallel`], so a single
//! cancelled request drops out of every worker thread at once.
//!
//! # Filesystem walking
//!
//! All filesystem walking goes through the [`ignore`] crate, which honors
//! `.gitignore`, `.ignore`, global gitignore, and hidden-file rules out of
//! the box. Hidden files are still surfaced via `.hidden(false)`. The `.git`
//! subtree is excluded by both the per-walker `filter_entry` and a
//! defense-in-depth [`path_has_git_component`] check on every yielded path.
...
impl FsConfig {
    ...
    fn fuzzy_file_suggestions(
        ...
        // Resolve scoped sub-tree walks for prefixes that contain a
        // path delimiter (e.g. `@src/foo`, `@../outside/a`,
        // `@~/notes/`, `@/etc/passwd`): when the leading directory
        // segment exists, walk *only* that subtree and score the
        // tail of the query against entries inside it. This lets us
        // return matches that live outside the project base without
        // walking everything, and keeps the displayed paths anchored
        // to whatever the user typed (`../outside/...`, `~/notes/...`).
        let scoped = self.resolve_scoped_query(query);
        let (walker_root, walker_query, display_base): (PathBuf, &str, Option<&str>) =
            match scoped.as_ref() {
                Some(s) => (
                    s.base_dir.clone(),
                    s.query.as_str(),
                    Some(s.display_base.as_str()),
                ),
                None => (self.base_path.clone(), query, None),
            };

        let mut scored =
            walk_for_suggestions_parallel(&walker_root, walker_query, self.walker_limit, cancel);
    ...
    /// Split a query that contains a `/` into the leading directory
    /// segment (`display_base`) and the trailing file fragment
    /// (`query`), resolving the directory to a real `PathBuf` for
    /// the walker. Returns `None` if the query has no slash or the
    /// resolved directory does not exist.
    fn resolve_scoped_query(&self, raw_query: &str) -> Option<ScopedQuery> {
        let normalized = to_display_path(raw_query);
        let slash_index = normalized.rfind('/')?;
        let display_base = &normalized[..slash_index + 1];
        let query = &normalized[slash_index + 1..];
        ...
        Some(ScopedQuery {
            ...
            display_base: display_base.to_string(),
        })
    }
}

/// Resolved scoped fuzzy walk. See [`FsConfig::resolve_scoped_query`].
struct ScopedQuery {
    base_dir: PathBuf,
    query: String,
    display_base: String,
...
/// Re-attach a scoped walk's leading directory segment to a path
/// reported relative to the scoped base. Handles the
/// `display_base = "/"` case (root walk) by emitting `/<rel>`
/// instead of `//<rel>`.
fn scoped_path_for_display(display_base: &str, relative: &str) -> String {
    let normalized = to_display_path(relative);
    if display_base == "/" {
        format!("/{normalized}")
    } else {
...
impl AutocompleteProvider for CombinedAutocompleteProvider {
    ...
    fn try_start_session(
        ...
        // Out-of-tree prefixes (`@/etc/`, `@~/notes`, `@../foo`)
        // need a walker rooted outside the project base; the
        // streaming session can't serve them. Hand back to the
        // one-shot path, which uses `resolve_scoped_query` to walk
        // the targeted subtree directly.
        if is_out_of_tree_prefix(&parsed.raw_prefix) {
            return None;
        }
...
/// we use [`ignore::WalkBuilder::build_parallel`] so the walk fans out
/// across CPU cores.
///
...
fn walk_for_suggestions_parallel(
    ...
    let walker = WalkBuilder::new(root)
        ...
        .ignore(true)
        .parents(true)
        // Follow symlinks so `link.txt -> original.txt` is reported
        // as a regular file and `symlinked_dir -> ../outside` is
        // descended into. The crate handles loop detection
        // internally.
        .follow_links(true)
        .build_parallel();
...
/// True when any component of the absolute path is exactly `.git`.
///
/// Used by both walkers as a defense-in-depth `.git` exclusion. The
/// `ignore` crate's `filter_entry` already prevents the streaming
/// walker from descending into `.git`, and the parallel walker
/// scopes from the project base; this check additionally rejects
/// any entry whose absolute path crosses a `.git` directory anywhere
/// — including the case where the user explicitly scopes a fuzzy
/// query into `.git/` (`@.git/HEAD`, `@.git/refs/heads/...`).
///
/// This is intentionally stricter than walking from a `.git/` base
/// directory and then filtering only relative paths: `.git/` content
/// is tooling state that an attachment workflow has no business
/// surfacing, even when explicitly requested.
...
impl AutocompleteSession for FuzzyFileSession {
    ...
    fn update(
        ...
        // Same out-of-tree check as `try_start_session`: if the user
        // typed a `..`, `~/`, or `/` segment that re-roots the walk,
        // tear down the session so the editor falls back to the
        // one-shot scoped walker.
        if is_out_of_tree_prefix(&parsed.raw_prefix) {
            return Err(SessionInvalid);
        }
...
/// True if the raw `@` prefix would resolve outside the project
/// base — absolute, home-relative, or parent-relative. The
/// streaming session walks from a fixed root and can't serve these
/// contexts; the editor falls back to the one-shot path, which
/// scopes the walker per-request via
/// [`FsConfig::resolve_scoped_query`].
fn is_out_of_tree_prefix(raw_prefix: &str) -> bool {
...
/// injector.
///
/// A **sequential** walker (not `build_parallel`) pushes each entry
/// into the thread-safe injector as it's discovered. Sequential
/// walking pairs naturally with nucleo's own worker pool —
...
fn spawn_walker_task(
    ...
    tokio::task::spawn_blocking(move || {
        let walker = WalkBuilder::new(&base_dir)
            ...
            .ignore(true)
            .parents(true)
            // Mirror the parallel walker — see
            // `walk_for_suggestions_parallel` for the rationale.
            .follow_links(true)
