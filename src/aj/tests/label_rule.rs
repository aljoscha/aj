...
fn every_label_site_bounds_bytes_at_its_own_limit() {
    for site in sites() {
        ...
        // The bound is bytes, not characters: a two-byte character past the
        // half-way mark is refused while its character count is well inside.
        let multibyte = "é".repeat(site.max_bytes / 2 + 1);
        assert_eq!(
            (site.apply)(&multibyte),
            SiteResult::TooLong,
            "{}: the bound counts bytes",
            site.name
        );
    }
}
