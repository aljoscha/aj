...
| `CompactionEnd` | `lifecycle.compacting.remove(id)`. On `error`, append `Notice{Warning}` "Compaction failed: ...". On `summary` (success), append `CompactionEntry` keyed by the frame's durable entry and call `footers.set_context_tokens(id, tokens_after)`. When `usage` is present, append or update a `TurnUsage` row keyed by that same durable entry and advance cumulative spend without replacing context occupancy with summarizer input. Absent usage remains unknown and leaves prior spend intact. Replayed checkpoints update the same summary and usage rows without duplicating them or overwriting later assistant usage. On neither, append `Notice{Info}` "Compaction canceled." |
