...
| `CompactionUsageUpdate` | Append or update a `TurnUsage` row keyed directly by `checkpoint_id`. Advance cumulative spend without replacing context occupancy with the summarizer's prompt size. The event is independently idempotent when attach filtering drops a duplicate `CompactionEnd`, and older clients safely ignore the distinct additive type. |
...
| `CompactionEnd` | `lifecycle.compacting.remove(id)`. On `error`, append `Notice{Warning}` "Compaction failed: ...". On `summary` (success), append `CompactionEntry` keyed by the frame's durable entry and call `footers.set_context_tokens(id, tokens_after)`. Compaction usage owns its identity independently. On neither, append `Notice{Info}` "Compaction canceled." |
