...
The new shape mirrors how pi-mono splits things: the agent is a pure
state machine that emits a typed event stream; consumers (a TUI, a
plain-text CLI, an RPC server, log persistence, tests) subscribe.
...
agent core, not the other way around. This matches how the reference
codebase splits things — the contract types (events, tool trait, tool
outcome) live in the same package as the `Agent` runtime, and tool
implementations sit a layer up.
...
pub struct ToolOutcome {
    ...
    /// flagged with is_error=true on Anthropic, etc).
    pub is_error: bool,
}
...
prune-on-emit (drop closed senders), modeled on pi's listener set.
Listeners are awaited or polled out of band.
...
If we want gating later, the hook to add is `Agent::set_before_tool_call`
in the style of the reference codebase. Not in v1.
