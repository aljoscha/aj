...
The registry holds all known models, organized by provider. It's populated at
startup from a built-in model catalog (a Rust data structure, not a generated
file).
...
3. **Define model metadata and registry** (`aj-models::registry`): Create
   ...
   Populate with built-in Anthropic and OpenAI model entries. Implement
   `calculate_cost`.
